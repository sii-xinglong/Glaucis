"""GMM block-wise FP8 quantization kernel for TPU -- L2_scale_bf16 variant.

Builds on L1_bwd_tm256 (2.290x speedup) by changing scale_dtype from float32
to bfloat16 in the two forward qpl.quantize() calls (lhs and rhs).

Motivation: 92.4% of VLIW bundles (23,462 total) are non-MXU, dominated by
the 2 forward quantize calls. Each quantize call computes absmax, scales
(in float32), and casts to fp8. Using scale_dtype=jnp.bfloat16 reduces the
scale tensor size and may simplify VPU operations for scale computation.
Fewer float32 VPU operations means fewer VLIW bundles.

Key changes vs L1_bwd_tm256:
  - Forward lhs quantize: scale_dtype=jnp.float32 -> jnp.bfloat16
  - Forward rhs quantize: scale_dtype=jnp.float32 -> jnp.bfloat16
  - Everything else identical: same tiling, same _clamp_tiling, same zero
    backward quantization, same phase-specialized tiling.

Risk: may cause correctness issues if tokamax expects float32 scales internally.
Could also be a no-op if the scale is immediately cast anyway.
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 256, 128, 256, 256, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_scale_bf16 variant: uses bfloat16 scale_dtype in forward quantize calls
    to reduce VPU overhead from float32 scale computation. Phase-specialized
    tiling with fwd (256,256,128), bwd_gmm (1024,256,128), tgmm (2048,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # L2_scale_bf16: use bfloat16 scale_dtype to reduce VPU float32 operations
    lhs = qpl.quantize(
        lhs_bf16,
        qt_rule.act_qtype,
        channelwise_axes=[0],
    ...
    # Forward gmm with phase-specialized tiling: (256, 256, 128)
    # TM=256 matches per-group M exactly (M=8192 / G=32 = 256)
    # TK=256 uses smaller K tiles to reduce register pressure in the
    # inner loop -- fewer live intermediate values per K iteration
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (1024, 256, 128) -- proven optimal from SO8
