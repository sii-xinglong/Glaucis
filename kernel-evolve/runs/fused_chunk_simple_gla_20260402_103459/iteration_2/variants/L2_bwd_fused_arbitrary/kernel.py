...
# Variant: L2_bwd_fused_arbitrary — true monolithic backward with dh in VMEM scratch
"""fused_chunk_simple_gla Pallas TPU kernel — L1 + forward 4-step unrolling + true monolithic backward.

Forward kernel: 4-step grid unrolling (unchanged from base).

Backward: TRUE single-pass monolithic kernel.  One pallas_call computes ALL
gradients (dq, dk, dv) while propagating dh in VMEM scratch, iterating in
reverse time order with "arbitrary" semantics on the time dimension.

Key difference from Round 1's bwd_monolithic:
  - h_ref indexes directly into chunk_fwd_h output shape (B, NT, H, K, V)
    using a 5D BlockSpec — eliminates the h.transpose(0,2,1,3,4) copy.
  - dh lives exclusively in VMEM scratch (K, V) — never materialised in HBM.
  - Single reverse-time pass: dq, dk, dv, dh all computed per time step.

Optimization targets within the EVOLVE-BLOCK:
  - Forward kernel: 4-step time unrolling to reduce grid overhead (unchanged)
  - Backward kernel: true monolithic single-pass with zero HBM intermediates
"""
...
# =============================================================================
# Backward: TRUE single-pass monolithic kernel — dq + dk + dv + dh in one pass
# =============================================================================
# This backward kernel computes ALL gradients (dq, dk, dv) and propagates dh
# in a single reverse-time pass with "arbitrary" time dimension semantics.
#
# Key optimizations over Round 1's bwd_monolithic:
#   1. h_ref uses a 5D BlockSpec that indexes directly into chunk_fwd_h output
#      shape (B, NT, H, K, V) — eliminates h.transpose(0,2,1,3,4) copy.
#   2. dh lives exclusively in VMEM scratch (K, V) float32 — zero HBM
#      intermediates for backward state propagation.
#   3. Single pallas_call, single reverse-time pass.
#
# Per time step (reverse order, t=NT-1 down to t=0):
#   1. Read h from h_ref (pre-computed forward h states at this time step)
#   2. Read dh from VMEM scratch
#   3. Compute dv = (inter: k_decay @ dh) + (intra: A.T @ do)
#   4. Compute dq = (inter: do @ h.T * scale * exp(g)) + (intra: dA_gated @ k)
#   5. Compute dk = (inter: v @ dh.T * exp(gn-g)) + (intra: dA_gated.T @ q)
#   6. Update dh: dh = dh * exp(gn) + q_hat.T @ do
#   7. Write dq, dk, dv to output Refs


def _fused_chunk_bwd_kernel(
    q_ref,          # (1, 1, BT, K)    — reversed chunk order via index_map
    k_ref,          # (1, 1, BT, K)    — reversed chunk order
    v_ref,          # (1, 1, BT, V)    — reversed chunk order
    h_ref,          # (1, 1, 1, K, V)  — from chunk_fwd_h, 5D BlockSpec, reversed
    ...
    *,
    ...
    """True monolithic backward kernel: dh propagation + dq/dk/dv in one pass.
    ...
    h_ref indexes directly into chunk_fwd_h output (B, NT, H, K, V) via
    a 5D BlockSpec — no transpose needed.

    At each step:
      1. Load dh from VMEM scratch (gradient flowing from future chunks).
      2. Recompute attention matrix A.
      3. Compute dv (intra + inter using h and dh).
      4. Compute dq and dk (intra + inter).
      5. Update dh for the previous chunk and save to scratch.
    """
    K = q_ref.shape[3]
    V = v_ref.shape[3]
    ...
    # ---- Recompute A in-kernel (avoids storing A from forward) ----
    ...
    # Recurrence: dh_{t-1} = dh_t * exp(g_gamma * BT) + (q*scale*exp(g))^T @ do
...
def _fused_chunk_bwd_launcher(
    ...
    """True single-pass monolithic backward launcher.
    ...
    accesses h[b, NT-1-t, h, :, :] directly.
    ...
    # Reversed chunk order for q/k (B, H, T, K) layout
    def rev_qk_map(b, h, t):
        return (b, h, NT - 1 - t, 0)

    # Reversed chunk order for v/do (B, H, T, V) layout
    def rev_v_map(b, h, t):
        return (b, h, NT - 1 - t, 0)

    # h is (B, NT, H, K, V) — 5D BlockSpec with reverse-time access
    # index_map returns (b, NT-1-t, h, 0, 0) to access h[b, chunk_idx, h, :, :]
    def rev_h_map(b, h, t):
        return (b, NT - 1 - t, h, 0, 0)
    ...
    in_specs = [
        ...
        pl.BlockSpec((1, 1, 1, K, V), rev_h_map),                           # h — 5D direct
    ...
    dq, dk, dv, dh0 = pl.pallas_call(
        ...
        out_shape=out_shapes,
        compiler_params=pltpu.CompilerParams(
            # NT is arbitrary — sequential backward propagation of dh
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    Backward uses a TRUE monolithic fused kernel that computes dq, dk, dv,
    and propagates dh all at once. h_ref indexes directly into chunk_fwd_h
    output (B, NT, H, K, V) via 5D BlockSpec — eliminates h transpose.
    ...
    def _bwd(residuals, do):
        ...
        # Single fused backward pass — no intermediate dh_states tensor
