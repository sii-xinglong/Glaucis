"""GMM block-wise FP8 quantization kernel for TPU -- tgmm_tm4096 variant.

Builds on L1_fwd_tk512 (2.335x) with increased tgmm TM from 2048 to 4096.

With bf16 tgmm (no fp8 constraints), TM can be pushed beyond 2048.
TM=4096 halves M-tiles from 4 to 2 (M=8192/4096=2), reducing kernel launch
overhead and M-loop iterations. FP7 (tgmm M>1024 breaks fp8 scale shapes)
does NOT apply because we use bf16 tgmm (SO6).

Trade-off: larger accumulator (4096*128*4bytes = 2MB per tile) may increase
VMEM pressure, but fewer M-tiles means less loop overhead.

Key changes vs L1_fwd_tk512:
  - tgmm TM: 2048 -> 4096 (position 6 in tiling tuple)
  - Everything else identical: same forward fp8 quantization, same zero
    backward quantization, same _clamp_tiling with max(t, 128)

Phase-specialized tiling: (256,512,128, 1024,256,128, 4096,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 1024, 256, 128, 4096, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    tgmm_tm4096 variant: phase-specialized tiling with
    fwd (256,512,128), bwd_gmm (1024,256,128), tgmm (4096,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (4096, 512, 128) -- TM=4096 to halve M-tiles from 4 to 2.
    # Since tgmm operates in bf16 (not fp8), larger TM is safe.
    # Fewer M-tiles reduces kernel launch overhead at the cost of larger
    # accumulator (4096*128*4bytes = 2MB per tile).
