...
# EVOLVE-BLOCK-START
def _clamp_tiling(tiling_slice, tile_size, use_fp8):
    """Clamp tiling dimensions to tile_size only for FP8 phases.

    FP8 phases require tiling <= tile_size due to scale tensor constraints.
    bf16 phases are unconstrained and can use larger tiles for better performance.
    """
    if use_fp8:
        return tuple(min(t, tile_size) for t in tiling_slice)
    return tuple(tiling_slice)
...
def gmm_fp8_blockwise(
    ...
    Zero backward quantization variant (SO8): eliminates ALL quantization
    from the backward pass while keeping FP8 for the forward pass.
    """
    tile_size = 128
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Clamp forward tiling for FP8 phase
    fwd_tiling = _clamp_tiling(tiling[:3], tile_size, use_fp8=True)

    # Keep bf16 lhs for backward pass (SO5+SO7: avoid pre-quantizing lhs_t)
    lhs_bf16 = lhs

    # Forward: quantize lhs to FP8 for forward GMM
    lhs = qpl.quantize(
        lhs_bf16,
        qt_rule.act_qtype,
        channelwise_axes=[0],
    ...
    # Residual: store lhs_bf16 (not pre-quantized lhs_t) and fp8 rhs
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # bwd_gmm tiling: FP8 rhs still involved, clamp to tile_size (SO7)
    bwd_gmm_tiling = _clamp_tiling(
        tiling[3:6] if len(tiling) >= 6 else tiling[:3],
        tile_size,
        use_fp8=True,
    )

    # tgmm tiling: all bf16, unconstrained (SO6)
    tgmm_tiling = _clamp_tiling(
        tiling[6:9] if len(tiling) >= 9 else tiling[:3],
        tile_size,
        use_fp8=False,
    )

    # SO7: pass bf16 grad directly to bwd_gmm -- no dlhs_dout quantization
    # rhs is still fp8 from forward, so mixed precision GMM
    ...
    # SO5+SO6: bf16 tgmm -- compute lhs_t from stored bf16 lhs, pass bf16 grad
    # No quantization at all, unconstrained tiling (2048, 256, 128)
