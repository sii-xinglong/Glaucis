...
#
# MUTATION (L2_extra_scratch): Adds 3 extra VMEM scratch buffers to
# explicitly stage intermediate values that the compiler would
# otherwise keep in registers (causing 2.72M register spills).
#
# Extra scratch buffers:
#   scratch_exp [BT, BK] — stores exp_pos_g, computed in Phase 0,
#       needed again in Phase 5 for dq/dk gating. Frees registers
#       during the matmul-heavy Phases 1-4.
#   scratch_A [BT, BT] — stores masked A matrix from Phase 2,
#       needed in Phase 3 for dv_intra. Frees registers during
#       Phases 3-4.
#   scratch_dA [BT, BT] — stores masked dA matrix from Phase 2,
#       needed in Phase 4 for dq/dk_intra. Frees registers during
#       Phase 3.
#
# Total VMEM: 128KB (out of 128MB available on TPU v7x).
#
# Key insight: The register allocator spills these intermediates
# anyway (14.60% Vector Store from spills), but in an uncontrolled
# pattern. By explicitly staging to VMEM scratch, we give the
# compiler a clear signal about lifetimes, enabling better
# register allocation for the remaining values.
# ============================================================


def _chunk_gla_bwd_extra_scratch_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, do_ref, g_gamma,
    dq_ref, dk_ref, dv_ref,
    scratch_dh_ref, scratch_exp_ref, scratch_A_ref, scratch_dA_ref,
    *, BT, NT, scale,
):
    """Fused backward kernel with extra VMEM scratch for intermediate staging.

    Uses 4 scratch buffers:
      scratch_dh_ref [BK, BV] — dh state accumulator (same as base)
      scratch_exp_ref [BT, BK] — stages exp_pos_g (computed Phase 0, used Phase 5)
      scratch_A_ref [BT, BT] — stages masked A (computed Phase 2, used Phase 3)
      scratch_dA_ref [BT, BT] — stages masked dA (computed Phase 2, used Phase 4)

    This frees registers during the matmul-heavy Phases 3-4, reducing the
    2.72M register spills from the baseline kernel.
    """
    BK = q_ref.shape[3]
    BV = do_ref.shape[3]
    i_b, i_h, i_k, i_v, i_t = (
    ...
    @pl.when(i_t == 0)
    def init():
        scratch_dh_ref[:, :] = jnp.zeros((BK, BV), dtype=jnp.float32)
    ...
    # STAGE exp_pos_g to VMEM scratch — it won't be needed until Phase 5
    # This frees BT*BK = 64*128 = 8192 float32 registers during Phases 1-4
    scratch_exp_ref[:, :] = exp_pos_g.astype(jnp.float32)
    ...
    # -------------------------------------------------------
    # Phase 2 (VPU): Apply causal masks and stage to VMEM scratch
    # -------------------------------------------------------
    mask = jnp.arange(BT)[:, None] >= jnp.arange(BT)[None, :]
    b_dA = jnp.where(mask, b_dA_raw, 0.0)
    b_a_masked = jnp.where(mask, b_a, 0.0)

    # STAGE masked A and dA to VMEM scratch — frees BT*BT*2 = 64*64*2 = 8192
    # float32 values from registers during Phase 3-4 matmuls
    scratch_A_ref[:, :] = b_a_masked.astype(jnp.float32)
    scratch_dA_ref[:, :] = b_dA.astype(jnp.float32)

    # -------------------------------------------------------
    # Phase 3 (MXU batch): dv (intra + inter) and dq_inter, dk_inter
    # Now b_a_masked and b_dA are in VMEM scratch, not registers.
    # -------------------------------------------------------
    # Reload b_a_masked from scratch for dv_intra
    b_a_staged = scratch_A_ref[:, :].astype(jnp.float32)

    b_dv_intra = jnp.dot(b_a_staged.T.astype(b_do.dtype), b_do,
                         precision=jax.lax.Precision.HIGHEST,
    ...
    # Reload b_dA from scratch — it was staged in Phase 2
    # -------------------------------------------------------
    b_dA_staged = scratch_dA_ref[:, :].astype(jnp.float32)

    b_dq_intra_raw = jnp.dot(b_dA_staged.astype(k_neg.dtype), k_neg,
                              precision=jax.lax.Precision.HIGHEST,
                              preferred_element_type=jnp.float32)  # [BT, K]

    b_dk_intra_raw = jnp.dot(b_dA_staged.T.astype(q_pos.dtype), q_pos,
                              precision=jax.lax.Precision.HIGHEST,
                              preferred_element_type=jnp.float32)  # [BT, K]

    # -------------------------------------------------------
    # Phase 5 (VPU): Combine results and write to 5D output slots
    # Reload exp_pos_g from VMEM scratch for gating dq/dk
    # -------------------------------------------------------
    exp_pos_g_reloaded = scratch_exp_ref[:, :].astype(jnp.float32)  # [BT, K]
    # Recompute exp_neg_g and exp_gn_minus_g from exp_pos_g (cheaper than storing)
    exp_neg_g_reloaded = 1.0 / exp_pos_g_reloaded                   # [BT, K]
    exp_gn_minus_g_reloaded = jnp.exp(b_gn[None, :]) * exp_neg_g_reloaded  # [BT, K]

    dv_ref[0, i_t, 0] = (b_dv_intra + b_dv_inter).astype(dv_ref.dtype)

    b_dq = b_dq_intra_raw * exp_pos_g_reloaded + b_dq_inter * (scale * exp_pos_g_reloaded)
    dq_ref[0, i_t, 0] = b_dq.astype(dq_ref.dtype)

    b_dk = b_dk_intra_raw * exp_neg_g_reloaded + b_dk_inter * exp_gn_minus_g_reloaded
    ...
    # -------------------------------------------------------
    scratch_dh_ref[...] *= exp(b_g_last)

    q_hat = (b_q * exp(b_g_ramp)[:, None] * scale).astype(jnp.float32)

    scratch_dh_ref[...] = scratch_dh_ref[...] + jax.lax.dot(
        q_hat.T,                           # [BK, BT]
...
def chunk_gla_bwd_recompute_dh_v2(q, k, v, g_cumsum, h, do, g_gamma, scale, chunk_size):
    """Fused backward with extra VMEM scratch buffers for intermediate staging.

    MUTATION (L2_extra_scratch): Adds 3 extra scratch buffers to explicitly
    stage intermediate values (exp_pos_g, masked A, masked dA) in VMEM.
    This gives the compiler clear lifetime information, reducing the 2.72M
    register spills from uncontrolled intermediate placement.

    Scratch layout:
      scratch_dh  [BK, BV]  = [128, 128] f32 — dh state accumulator (64KB)
      scratch_exp [BT, BK]  = [64, 128]  f32 — exp_pos_g staging (32KB)
      scratch_A   [BT, BT]  = [64, 64]   f32 — masked A staging (16KB)
      scratch_dA  [BT, BT]  = [64, 64]   f32 — masked dA staging (16KB)
      Total: 128KB VMEM (out of 128MB available on TPU v7x)
    """
    BK, BV, BT = 128, 128, chunk_size
    B, T, H, K = q.shape
    V = v.shape[-1]
    ...
    dq_5d, dk_5d, dv_5d = pl.pallas_call(
        functools.partial(
            _chunk_gla_bwd_extra_scratch_kernel,
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            scratch_shapes=[
                pltpu.VMEM((BK, BV), jnp.float32),   # scratch_dh: [128, 128] dh accumulator
                pltpu.VMEM((BT, BK), jnp.float32),   # scratch_exp: [64, 128] exp_pos_g staging
                pltpu.VMEM((BT, BT), jnp.float32),   # scratch_A: [64, 64] masked A staging
                pltpu.VMEM((BT, BT), jnp.float32),   # scratch_dA: [64, 64] masked dA staging
            ],
        ),
        out_shape=[
...
# custom_vjp wrapper
#
# MUTATION (L2_extra_scratch + fuse_fwd_combined):
#   _fwd: uses chunk_fwd_combined (single pallas_call for forward)
#   _bwd: uses chunk_gla_bwd_recompute_dh_v2 with 4 VMEM scratch
#         buffers (dh + exp + A + dA) to reduce register spills
#
# Total pallas_calls: 2 (1 fwd + 1 bwd)
# ============================================================
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        # MUTATION (L2_extra_scratch): Single fused pallas_call with
        # 4 VMEM scratch buffers to reduce register spills from 2.72M
        # by explicitly staging exp_pos_g, masked A, and masked dA.
