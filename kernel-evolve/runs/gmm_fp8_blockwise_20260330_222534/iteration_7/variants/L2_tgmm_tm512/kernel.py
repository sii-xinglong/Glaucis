"""GMM block-wise FP8 quantization kernel for TPU -- L2_tgmm_tm512 variant.

Builds on L2_tgmm_tm1024 (iteration 6) which decreased tgmm TM from 2048 to
1024 and achieved +2.8% improvement. This variant pushes further, halving tgmm
TM again from 1024 to 512 to explore whether the trend continues.

Hypothesis: TM=2048->1024 gave +2.8% by reducing register pressure and memory
traffic in the tgmm M-loop. If the bottleneck was tile size overhead, TM=512
should yield additional gains. If TM=1024 was already near-optimal, returns
will diminish or performance may regress due to increased loop iteration count.

Tiling changes vs parent (L2_tgmm_tm1024):
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (512, 512, 128) -- TM 1024->512
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,128, 512,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 512, 128, 512, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_tgmm_tm512 variant: tgmm TM 1024->512 to test if smaller TM
    continues the improvement trend (TM=2048->1024 gave +2.8%).
    fwd (256,512,256), bwd_gmm (256,512,128), tgmm (512,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (512, 512, 128) -- TM 1024->512 to test if smaller TM
    # continues the improvement trend from TM=2048->1024 (+2.8%).
