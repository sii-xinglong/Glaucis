"""GMM block-wise FP8 quantization kernel for TPU -- L1_all_tn256 variant.
...
This variant increases TN from 128 to 256 in BOTH forward and backward gmm
passes. This doubles N-tile size across both phases, halving N-loop iterations
in both forward and backward. This tests whether TN=256 benefits compound
when applied to both phases simultaneously.
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,256, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 512, 256, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_all_tn256 variant: both fwd and bwd TN 128->256 to halve N-loop iterations.
    fwd (256,512,256), bwd_gmm (256,512,256), tgmm (2048,512,128).
