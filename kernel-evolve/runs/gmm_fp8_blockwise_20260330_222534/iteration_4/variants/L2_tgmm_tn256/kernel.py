"""GMM block-wise FP8 quantization kernel for TPU -- tgmm_tn256 variant.

Builds on L2_bwd_tm256_tk512 (parent) which proved TM=256 and TK=512 for
bwd_gmm. This variant increases tgmm TN from 128 to 256, halving the N-grid
iterations for the tgmm phase.

The tgmm phase operates entirely in bf16 (SO6), so there are no fp8
quantization tile constraints on TN. TN=256 was already proven to work for
fwd and bwd_gmm tiling dimensions.

Phase-specialized tiling:
  - Forward gmm:  (256, 256, 128) -- TM=256 matches per-group M exactly,
    TK=256 for less register pressure (smaller K tiles)
  - Backward gmm: (256, 512, 128) -- TM=256 for per-group alignment +
    TK=512 halves K-loop iterations (K=2048: 4 iters instead of 8)
  - tgmm:         (2048, 512, 256) -- TN=256 halves N-grid iterations;
    bf16 precision means no fp8 tile constraints apply (SO6)
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,256,128, 256,512,128, 2048,512,256)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 256, 128, 256, 512, 128, 2048, 512, 256),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    tgmm_tn256 variant: phase-specialized tiling with
    fwd (256,256,128), bwd_gmm (256,512,128), tgmm (2048,512,256).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (2048, 512, 256) -- TN=256 halves N-grid iterations.
    # Since tgmm operates in bf16 (not fp8), larger N tiles are safe and
    # reduce grid overhead without fp8 quantization tile constraints (SO6).
