"""GMM block-wise FP8 quantization kernel for TPU -- fwd_tk512_scale_bf16 variant.

Builds on L2_bwd_tm256_tk512 which combined TM=256 with TK=512 for bwd_gmm.
This variant makes two changes to the forward path:
  1. Forward TK: 256 -> 512, halving K-loop iterations in the forward gmm
     (K=2048: 4 iters instead of 8, K=512: 1 iter instead of 2)
  2. Scale dtype: float32 -> bfloat16 for both lhs and rhs quantization scales,
     reducing scale memory footprint and enabling faster scale arithmetic on MXU
...
Tiling: (256, 512, 128, 256, 512, 128, 2048, 512, 128)
  - Forward gmm:  (256, 512, 128) -- TM=256 matches per-group M exactly,
    TK=512 halves K-loop iterations for forward pass
...
Key changes vs baseline:
  ...
  - Forward: scale_dtype=jnp.bfloat16 for both quantize calls (saves memory)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 256, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    fwd_tk512_scale_bf16 variant: phase-specialized tiling with
    fwd (256,512,128), bwd_gmm (256,512,128), tgmm (2048,512,128).
    Zero backward quantization with mixed-precision bwd_gmm.
    bf16 scale dtype for forward quantization.
    """
    tile_size = 128
