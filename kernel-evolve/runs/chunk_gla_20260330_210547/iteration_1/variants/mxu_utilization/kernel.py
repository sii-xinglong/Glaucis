...
# ============================================================
# Backward: Split kernel A — dq and dv (share b_do operand)
# ============================================================
# By splitting the fused backward into two separate kernels we halve the
# number of live values per kernel. This gives the Mosaic compiler enough
# register headroom to schedule the two dot products inside each kernel
# onto mxu0 and mxu1 simultaneously, targeting dual_ratio > 0.
#
# Kernel A owns: dq_intra (b_dA @ k_neg) + dq_inter (b_do @ b_h.T)
#                dv_intra (b_a.T @ b_do) + dv_inter (k_decay @ b_dh)
# Both dq paths consume b_dA or b_do; both dv paths consume b_do or b_dh.
# ============================================================


def _chunk_gla_bwd_dq_dv_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, a_ref, do_ref, dh_ref,
    dq_ref, dv_ref,
    *, BT, scale,
):
    """Compute dq and dv only. Fewer live values → compiler can dual-schedule MXUs."""
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    b_g = g_ref[0, 0].astype(jnp.float32)
    ...
    # dA = (b_do @ b_v.T) * scale, masked causal
    b_dA = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                   precision=jax.lax.Precision.HIGHEST,
                   preferred_element_type=jnp.float32) * scale
    b_dA = jnp.where(mask, b_dA, 0.0)

    # dv: two independent dot products — candidates for dual-MXU scheduling
    b_a_masked = jnp.where(mask, b_a, 0.0)
    # dv_intra uses b_a_masked (BT×BT) @ b_do (BT×V): independent of dv_inter
    b_dv_intra = jnp.dot(b_a_masked.T.astype(b_do.dtype), b_do,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)
    # dv_inter uses k_decay (BT×K) @ b_dh (K×V): different operand set from dv_intra
    ...
    # dq: two independent dot products — candidates for dual-MXU scheduling
    k_neg = (b_k * jnp.exp(-b_g)).astype(b_k.dtype)
    # dq_intra uses b_dA (BT×BT) @ k_neg (BT×K): independent of dq_inter
    b_dq_intra = jnp.dot(b_dA.astype(k_neg.dtype), k_neg,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32) * jnp.exp(b_g)
    # dq_inter uses b_do (BT×V) @ b_h.T (V×K): different operand set
...
# ============================================================
# Backward: Split kernel B — dk and dg (share b_dA and b_k)
# ============================================================
# Kernel B owns: dk_intra (b_dA.T @ q_pos) + dk_inter (b_v @ b_dh.T)
#                dg computed from dk and dq results
# dk_intra and dk_inter use completely different operand sets → dual-MXU.
# ============================================================


def _chunk_gla_bwd_dk_dg_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, a_ref, do_ref, dh_ref,
    dq_ref,
    dk_ref, dg_ref,
    *, BT, scale,
):
    """Compute dk and dg only. Receives dq from kernel A for dg accumulation."""
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    b_g = g_ref[0, 0].astype(jnp.float32)
    ...
    b_do = do_ref[0, 0]
    b_dh = dh_ref[0, 0].astype(jnp.float32)
    b_dq = dq_ref[0, 0].astype(jnp.float32)

    b_gn = b_g[BT - 1, :]
    mask = jnp.arange(BT)[:, None] >= jnp.arange(BT)[None, :]

    # Recompute dA (needed for dk_intra) — cheaper than passing it between kernels
    b_dA = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                   precision=jax.lax.Precision.HIGHEST,
                   preferred_element_type=jnp.float32) * scale
    b_dA = jnp.where(mask, b_dA, 0.0)

    # dk: two independent dot products — candidates for dual-MXU scheduling
    q_pos = (b_q * jnp.exp(b_g)).astype(b_q.dtype)
    # dk_intra uses b_dA.T (BT×BT) @ q_pos (BT×K): independent of dk_inter
    b_dk_intra = jnp.dot(b_dA.T.astype(q_pos.dtype), q_pos,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32) * jnp.exp(-b_g)
    # dk_inter uses b_v (BT×V) @ b_dh.T (V×K): completely different operand set
...
def chunk_gla_bwd_split(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    """Launch split backward: kernel A (dq+dv) then kernel B (dk+dg).

    Splitting the original fused kernel into two smaller kernels reduces
    the number of simultaneously-live values per kernel, which is the
    prerequisite for the Mosaic compiler to schedule dot products across
    both MXU units (mxu0 and mxu1) simultaneously.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    # Kernel A: dq + dv
    dq, dv = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dq_dv_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
        ...
        out_specs=[spec_K, spec_V],
        compiler_params=pltpu.CompilerParams(disable_bounds_checks=True),
    )(_q, _k, _v, _g, _h, _A, _do, _dh)

    # Kernel B: dk + dg (reads dq from kernel A)
    dk, dg = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dk_dg_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
            jax.ShapeDtypeStruct([H, total_NT, BT, K], k.dtype),
            jax.ShapeDtypeStruct([H, total_NT, BT, K], g_cumsum.dtype),
        ],
        in_specs=[spec_K, spec_K, spec_V, spec_K, spec_h, spec_A, spec_V, spec_h, spec_K],
        out_specs=[spec_K, spec_K],
        compiler_params=pltpu.CompilerParams(disable_bounds_checks=True),
    )(_q, _k, _v, _g, _h, _A, _do, _dh, dq)
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        dq, dk, dv, _ = chunk_gla_bwd_split(q, k, v, g_cumsum, h, do, dh, scale, C)
        return dq, dk, dv
