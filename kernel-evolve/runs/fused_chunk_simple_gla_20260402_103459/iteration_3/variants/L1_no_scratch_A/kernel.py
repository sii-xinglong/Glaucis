...
# Variant: L1_no_scratch_A — cleanup: remove counterproductive VMEM scratch_A_ref
"""fused_chunk_simple_gla Pallas TPU kernel — L1_no_scratch_A variant.
...
Forward kernel: 4-step grid unrolling WITHOUT VMEM scratch for attention matrix A.
Grid is (B, H, NK, NV, NT//4) instead of (B, H, NK, NV, NT).

Cleanup variant: Removes the scratch_A_ref VMEM buffer that was added in L1_scratch_A.
Profile analysis showed scratch_A_ref added +224 VLIW bundles (+4.2%), +32 DMA ops
(+6.9%), +32KB VMEM with ZERO spill reduction (identical 6.3M spills). The [BT,BT]
attention matrix was NOT the primary source of register pressure, so staging it through
VMEM was pure overhead. This variant uses partial_A directly in the dot product.
...
# No VMEM scratch for attention matrix (cleanup: scratch_A_ref removed)
# =============================================================================
