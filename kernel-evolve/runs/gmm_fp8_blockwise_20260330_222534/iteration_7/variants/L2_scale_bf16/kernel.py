...
Cross-pollination of SO14 (tgmm TM=1024) with SO15 (bf16 scales).

Parent: L2_tgmm_tm1024 (iteration 6) which reduced tgmm TM from 2048 to 1024.
This variant changes scale_dtype from float32 to bfloat16 in the forward pass
quantization calls, combining two independent optimization directions:

  SO14 -- VLIW simplification from smaller tgmm TM=1024 (fewer M-loop iters,
          reduced register pressure in the tgmm pass).
  SO15 -- Register pressure elimination from bf16 scale_dtype. Float32 scales
          require 2x the register space vs bf16 and force f32 accumulation in
          scale application. Bfloat16 scales halve scale tensor size and let
          the Mosaic compiler keep everything in bf16 registers.

Tiling unchanged from parent:
  - Forward gmm:  (256, 512, 256)
  - Backward gmm: (256, 512, 128)
  - tgmm:         (1024, 512, 128)
...
Key changes vs baseline:
  ...
  - Forward quantization: scale_dtype=bfloat16 (SO15, was float32)
...
def gmm_fp8_blockwise(
    ...
    L2_scale_bf16 variant: cross-pollination of SO14 (tgmm TM=1024) with
    SO15 (bf16 scale_dtype). Combines VLIW simplification from smaller tgmm
    tiles with register pressure elimination from bf16 scales.
    fwd (256,512,256), bwd_gmm (256,512,128), tgmm (1024,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # SO15: use bfloat16 scales to halve scale tensor size and reduce
    # register pressure vs float32 scales
    lhs = qpl.quantize(
        lhs_bf16,
        qt_rule.act_qtype,
        channelwise_axes=[0],
