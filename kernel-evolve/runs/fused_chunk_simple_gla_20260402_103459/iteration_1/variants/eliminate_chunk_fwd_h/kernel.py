...
#   The kernel now ALSO writes h states at chunk boundaries to h_ref, so the
#   separate chunk_fwd_h pallas_call in _fwd is no longer needed.
#
# h_ref layout: (B, NT, H, K, V) accessed as block (1, 4, 1, BK, BV)
# Each NT_QUARTER iteration writes 4 consecutive h-state slots (one per sub-step,
# capturing h BEFORE each chunk update, which is what backward needs).
# =============================================================================
...
def _fused_chunk_fwd_4step_kernel(
    ...
    h_ref,          # (1, 4, 1, BK, BV) — h states at each of 4 chunk starts
    ...
    *,
    ...
    # Capture h BEFORE processing chunk 0 (this is what backward needs)
    ...
    # Capture h BEFORE processing chunk 1
    ...
    # Capture h BEFORE processing chunk 2
    ...
    # Capture h BEFORE processing chunk 3
...
def fused_chunk_fwd_kernel(
    ...
    *,
    ...
    def o_map(b, h, ik, iv, t):
        return (b, h, ik, t, iv)
    # h output: shape (B, NT, H, K, V) stored as (B, NT_QUARTER*4, H, BK, BV) block-tiled
    # Block covers 4 consecutive NT slots for each NT_QUARTER iteration.
    # h_ref inside kernel has shape (1, 4, 1, BK, BV).
    # index_map: (b, h, ik, iv, t) -> (b, 4*t, h, ik, iv)
    def h_out_map(b, h, ik, iv, t):
        return (b, 4 * t, h, ik, iv)
    ...
    # h output block: (1, 4, 1, BK, BV) — 4 consecutive chunk h-states per iteration
    spec_h_out = pl.BlockSpec((1, 4, 1, BK, BV), h_out_map)

    # out_shape for o_partial: time dim stays T (NT_QUARTER * 4*BT = NT * BT = T)
    # h output shape: (B, NT, H, K, V) — one h state per chunk, matching chunk_fwd_h output
    out_shapes = [
        jax.ShapeDtypeStruct((B, H, NK, T, V), jnp.float32),
        jax.ShapeDtypeStruct((B, H, K, V), jnp.float32)
        if output_final_state else None,
        jax.ShapeDtypeStruct((B, NT, H, K, V), jnp.bfloat16),
    ]

    o_partial, ht, h_states = pl.pallas_call(
        functools.partial(
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            in_specs=[spec_qk, spec_qk, spec_v, spec_h0,
                      spec_g, spec_gamma],
            out_specs=[spec_o, spec_ht, spec_h_out],
    ...
    # h_states shape: (B, NT, H, K, V) — needs to be summed over NK dimension.
    # The kernel writes to different NK blocks independently; NK tiles cover
    # different BK slices of the K dimension. The h_out_map maps ik -> ik,
    # so different NK iterations write to different K slices correctly.
    # No sum needed — NK dimension is the K tile index, already laid out correctly.
    # h_states already has shape (B, NT, H, K, V) with all K slices filled.

    return o, ht, h_states
...
# =============================================================================
# State propagation: chunk_fwd_h (kept for reference; NO LONGER CALLED in _fwd)
# The fused forward kernel now outputs h states directly.
# =============================================================================
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    h is now output directly from the fused forward kernel -- the separate
    chunk_fwd_h pallas_call has been eliminated.
    ...
    def _fwd(q, k, v):
        # fused_chunk_simple_gla_fwd now returns (o, ht, h_states)
        # h_states has shape (B, NT, H, K, V) — same as chunk_fwd_h output
        # This eliminates the separate chunk_fwd_h pallas_call entirely.
        o, _, h = fused_chunk_simple_gla_fwd(
            q, k, v, g_gamma=g_gamma, scale=scale, chunk_size=chunk_size,
        )
        # h is bfloat16 from the fused kernel; cast to float32 for backward
        # (chunk_fwd_h used states_in_fp32=True, matching backward expectations)
    ...
    def _bwd(residuals, do):
        ...
        # Use pre-computed h from fused forward — skip separate chunk_fwd_h call
