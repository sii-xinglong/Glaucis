...
# Variant: L2_bwd_2step — monolithic backward with 2-step grid unrolling
"""fused_chunk_simple_gla Pallas TPU kernel — L1 + forward 4-step unrolling + backward 2-step grid unrolling.

Forward kernel: 4-step grid unrolling (unchanged from base).

Backward: TRUE single-pass monolithic kernel with 2-step grid unrolling.
Each grid iteration processes 2 consecutive time chunks (in reverse order),
halving the grid dimension from NT to NT_HALF = NT // 2.

Key differences from L2_bwd_fused_arbitrary:
  - Grid (B, H, NT_HALF) instead of (B, H, NT)
  - BlockSpecs tile 2*BT in the time dimension
  - h_ref loads (1, 2, 1, K, V) blocks for 2 consecutive h states
  - Kernel body processes sub-step 0 (later chunk) then sub-step 1 (earlier chunk)
  - dh propagates between sub-steps via VMEM scratch

Optimization targets within the EVOLVE-BLOCK:
  - Forward kernel: 4-step time unrolling to reduce grid overhead (unchanged)
  - Backward kernel: 2-step grid unrolling on monolithic backward
"""
...
# =============================================================================
# Backward: Monolithic kernel with 2-step grid unrolling
# =============================================================================
# Same monolithic backward as L2_bwd_fused_arbitrary, but with 2-step grid
# unrolling: each grid iteration processes 2 consecutive time chunks in
# reverse order. Grid changes from (B, H, NT) to (B, H, NT_HALF).
#
# Within each 2-step window (covering chunks at original indices 2*t and 2*t+1
# in reverse order):
#   Sub-step 0: processes the LATER chunk (higher time index) first
#   Sub-step 1: processes the EARLIER chunk (lower time index) second
# This maintains correct backward (reverse-time) ordering.
#
# dh state propagates through VMEM scratch between sub-steps and across
# grid iterations.


def _fused_chunk_bwd_2step_kernel(
    q_ref,          # (1, 1, 2*BT, K)    — 2 chunks, reversed order via index_map
    k_ref,          # (1, 1, 2*BT, K)    — 2 chunks, reversed order
    v_ref,          # (1, 1, 2*BT, V)    — 2 chunks, reversed order
    h_ref,          # (1, 2, 1, K, V)    — 2 consecutive h states, reversed order
    do_ref,         # (1, 1, 2*BT, V)    — 2 chunks, reversed order
    g_gamma_ref,    # [H] via SMEM/ANY
    dht_ref,        # (1, 1, K, V) or None — terminal state gradient
    # --- outputs ---
    dq_ref,         # (1, 1, 2*BT, K)    — 2 chunks, reversed order
    dk_ref,         # (1, 1, 2*BT, K)    — 2 chunks, reversed order
    dv_ref,         # (1, 1, 2*BT, V)    — 2 chunks, reversed order
    ...
    *,
    ...
    """Monolithic backward kernel with 2-step grid unrolling.

    Grid: (B, H, NT_HALF) — B, H parallel, NT_HALF arbitrary (backward).

    The BlockSpec index_map reverses the 2-chunk windows:
      i_t=0 -> last 2 chunks (NT-2, NT-1)
      i_t=NT_HALF-1 -> first 2 chunks (0, 1)

    Within each window, the data is laid out as:
      q_ref[..., 0:BT, :] = the LATER chunk (higher time index)
      q_ref[..., BT:2*BT, :] = the EARLIER chunk (lower time index)
    This is because the index_map returns (NT_HALF-1-t) * 2 for q/k/v/do,
    and the later chunk has the higher offset in the reversed layout.

    Actually, let me reconsider. The q/k/v arrays are (B, H, T, D) with T
    in forward order. The BlockSpec tiles 2*BT and the index_map maps
    i_t -> (NT_HALF - 1 - i_t). So:
      i_t=0 -> offset (NT_HALF-1) in units of 2*BT -> last 2 chunks
      Within that 2*BT block:
        offset 0:BT = chunk at time (NT_HALF-1)*2 = NT-2
        offset BT:2*BT = chunk at time (NT_HALF-1)*2+1 = NT-1

    For backward processing, we need to process NT-1 first, then NT-2.
    So sub-step 0 processes offset BT:2*BT (the later chunk).
    Sub-step 1 processes offset 0:BT (the earlier chunk).

    Similarly for h_ref: shape (1, 2, 1, K, V).
      h_ref[0, 0, 0] = h at time (NT_HALF-1-i_t)*2
      h_ref[0, 1, 0] = h at time (NT_HALF-1-i_t)*2 + 1
    Sub-step 0 uses h_ref[0, 1, 0] (the later h state).
    Sub-step 1 uses h_ref[0, 0, 0] (the earlier h state).
    """
    K = q_ref.shape[3]
    V = v_ref.shape[3]
    i_t = pl.program_id(2)       # 0 -> last 2 chunks, NT_HALF-1 -> first 2 chunks
    head_idx = pl.program_id(1)
    ...
    # ---- Per-position decay from g_gamma (same for both sub-steps) ----
    b_gamma = g_gamma_ref[head_idx].astype(jnp.float32)
    ...
    # ======================================================================
    # Sub-step 0: process the LATER chunk (offset BT:2*BT in the 2-chunk window)
    # This is the chunk with the higher time index, processed first in backward.
    # ======================================================================
    b_q = q_ref[0, 0, pl.ds(BT, BT), :]                        # (BT, K)
    b_k = k_ref[0, 0, pl.ds(BT, BT), :]                        # (BT, K)
    b_v = v_ref[0, 0, pl.ds(BT, BT), :]                        # (BT, V)
    b_h = h_ref[0, 1, 0].astype(jnp.float32)                    # (K, V) — later h state
    b_do = do_ref[0, 0, pl.ds(BT, BT), :]                       # (BT, V)
    ...
    dv_ref[0, 0, pl.ds(BT, BT)] = (b_dv_intra + b_dv_inter).astype(dv_ref.dtype)
    ...
    dq_ref[0, 0, pl.ds(BT, BT)] = (b_dq_intra + b_dq_inter).astype(dq_ref.dtype)
    ...
    dk_ref[0, 0, pl.ds(BT, BT)] = (b_dk_intra + b_dk_inter).astype(dk_ref.dtype)

    # Update dh for the next sub-step (earlier chunk)
    ...
    # ======================================================================
    # Sub-step 1: process the EARLIER chunk (offset 0:BT in the 2-chunk window)
    # This is the chunk with the lower time index, processed second in backward.
    # ======================================================================
    b_q1 = q_ref[0, 0, pl.ds(0, BT), :]                         # (BT, K)
    b_k1 = k_ref[0, 0, pl.ds(0, BT), :]                         # (BT, K)
    b_v1 = v_ref[0, 0, pl.ds(0, BT), :]                         # (BT, V)
    b_h1 = h_ref[0, 0, 0].astype(jnp.float32)                   # (K, V) — earlier h state
    b_do1 = do_ref[0, 0, pl.ds(0, BT), :]                        # (BT, V)
    b_dh1 = scratch_ref[...].astype(jnp.float32)                 # (K, V) — updated from sub-step 0
    ...
    b_dA1 = jnp.where(mask, b_dA1, 0.0)

    # dv = A^T @ do  +  k_decay @ dh
    b_a1_masked = jnp.where(mask, b_a1, 0.0).astype(b_do1.dtype)
    b_dv1_intra = jnp.dot(
        b_a1_masked.T, b_do1,
        preferred_element_type=jnp.float32,
    )
    k_decay1 = (b_k1 * jnp.exp(b_gn - b_g)[:, None]).astype(b_k1.dtype)
    b_dv1_inter = jnp.dot(
        k_decay1, b_dh1.astype(b_k1.dtype),
        preferred_element_type=jnp.float32,
    )
    dv_ref[0, 0, pl.ds(0, BT)] = (b_dv1_intra + b_dv1_inter).astype(dv_ref.dtype)

    # Gated dA
    b_dA1_gated = b_dA1 * jnp.exp(safe_g_diff)

    # dq = gated_dA @ k  +  do @ h^T * scale * exp(g)
    b_dq1_intra = jnp.dot(
        b_dA1_gated.astype(b_k1.dtype), b_k1,
        preferred_element_type=jnp.float32,
    )
    b_dq1_inter = jnp.dot(
        b_do1, b_h1.astype(b_do1.dtype).T,
        preferred_element_type=jnp.float32,
    ) * (scale * jnp.exp(b_g)[:, None])
    dq_ref[0, 0, pl.ds(0, BT)] = (b_dq1_intra + b_dq1_inter).astype(dq_ref.dtype)

    # dk = gated_dA^T @ q  +  v @ dh^T * exp(g_last - g)
    b_dk1_intra = jnp.dot(
        b_dA1_gated.T.astype(b_q1.dtype), b_q1,
        preferred_element_type=jnp.float32,
    )
    b_dk1_inter = jnp.dot(
        b_v1, b_dh1.astype(b_v1.dtype).T,
        preferred_element_type=jnp.float32,
    ) * jnp.exp(b_gn - b_g)[:, None]
    dk_ref[0, 0, pl.ds(0, BT)] = (b_dk1_intra + b_dk1_inter).astype(dk_ref.dtype)

    # Update dh for previous grid iteration
    b_dh1 = b_dh1 * jnp.exp(b_gn)
    b_q1_hat = (b_q1 * (scale * jnp.exp(b_g)[:, None])).astype(jnp.float32)
    b_dh1 = b_dh1 + jnp.dot(
        b_q1_hat.T, b_do1.astype(jnp.float32),
        precision=jax.lax.Precision.HIGHEST,
        preferred_element_type=jnp.float32,
    )
    scratch_ref[...] = b_dh1

    # ---- Store dh0 at last backward step (earliest chunks) ----
    @pl.when(i_t == NT_HALF - 1)
    def _store_dh0():
...
def _fused_chunk_bwd_launcher(
    ...
    """Monolithic backward launcher with 2-step grid unrolling.

    h is passed directly in chunk_fwd_h output shape (B, NT, H, K, V) --
    no transpose needed.  A 5D BlockSpec with reverse-time index_map
    accesses 2 consecutive h states per grid iteration.

    Grid: (B, H, NT_HALF) where NT_HALF = NT // 2.
    dh propagates through VMEM scratch -- zero HBM intermediates.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    assert NT % 2 == 0, f"NT={NT} must be divisible by 2 for 2-step backward unrolling"
    NT_HALF = NT // 2

    # Transpose q, k, v, do to (B, H, T, D) layout for the kernel
    _q = q.transpose(0, 2, 1, 3)
    _k = k.transpose(0, 2, 1, 3)
    _v = v.transpose(0, 2, 1, 3)
    _do = do.transpose(0, 2, 1, 3)

    # h is already in chunk_fwd_h output shape (B, NT, H, K, V)
    # NO transpose -- we index directly via the 5D BlockSpec
    _h = h

    grid = (B, H, NT_HALF)

    # Reversed 2-chunk windows for q/k (B, H, T, K) layout
    # i_t=0 -> last 2 chunks, i_t=NT_HALF-1 -> first 2 chunks
    # Tile size is 2*BT; offset in chunks-of-2: (NT_HALF - 1 - t)
    def rev_qk_map(b, h, t):
        return (b, h, NT_HALF - 1 - t, 0)

    # Reversed 2-chunk windows for v/do (B, H, T, V) layout
    def rev_v_map(b, h, t):
        return (b, h, NT_HALF - 1 - t, 0)

    # h is (B, NT, H, K, V) -- 5D BlockSpec loading 2 consecutive h states
    # i_t=0 -> last 2 h states at indices (NT-2, NT-1)
    # i_t=NT_HALF-1 -> first 2 h states at indices (0, 1)
    # Block size: (1, 2, 1, K, V), offset: (b, (NT_HALF-1-t)*2, h, 0, 0)
    def rev_h_map(b, h, t):
        return (b, (NT_HALF - 1 - t) * 2, h, 0, 0)

    # State tensors (dht, dh0) are (B, H, K, V)
    def state_map(b, h, t):
        return (b, h, 0, 0)
    ...
    in_specs = [
        pl.BlockSpec((1, 1, 2 * BT, K), rev_qk_map),                        # q — 2 chunks
        pl.BlockSpec((1, 1, 2 * BT, K), rev_qk_map),                        # k — 2 chunks
        pl.BlockSpec((1, 1, 2 * BT, V), rev_v_map),                         # v — 2 chunks
        pl.BlockSpec((1, 2, 1, K, V), rev_h_map),                            # h — 2 consecutive states
        pl.BlockSpec((1, 1, 2 * BT, V), rev_v_map),                         # do — 2 chunks
        pl.BlockSpec(memory_space=smem),                                      # g_gamma
        pl.BlockSpec((1, 1, K, V), state_map) if dht is not None else None,  # dht
    ]

    out_specs = [
        pl.BlockSpec((1, 1, 2 * BT, K), rev_qk_map),                        # dq — 2 chunks
        pl.BlockSpec((1, 1, 2 * BT, K), rev_qk_map),                        # dk — 2 chunks
        pl.BlockSpec((1, 1, 2 * BT, V), rev_v_map),                         # dv — 2 chunks
    ...
    dq, dk, dv, dh0 = pl.pallas_call(
        functools.partial(
            _fused_chunk_bwd_2step_kernel,
            BT=BT, NT=NT, NT_HALF=NT_HALF, scale=scale,
        ),
        grid_spec=pltpu.PrefetchScalarGridSpec(
            num_scalar_prefetch=0,
            grid=grid,
        ...
        out_shape=out_shapes,
        compiler_params=pltpu.CompilerParams(
            # NT_HALF is arbitrary -- sequential backward propagation of dh
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    Backward uses a monolithic fused kernel with 2-step grid unrolling that
    computes dq, dk, dv, and propagates dh all at once. Grid is NT//2
    instead of NT. h_ref indexes directly into chunk_fwd_h output
    (B, NT, H, K, V) via 5D BlockSpec -- eliminates h transpose.
    dh lives exclusively in VMEM scratch -- zero HBM intermediates.
    """
    @jax.custom_vjp
    def _compute(q, k, v):
    ...
    def _bwd(residuals, do):
        ...
        # h is (B, NT, H, K, V) from chunk_fwd_h -- passed directly, no transpose
        # Single fused backward pass with 2-step grid unrolling
