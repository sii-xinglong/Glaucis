...
#
# MUTATION (reduce_live_bwd): Restructure backward kernel to
# reduce peak live variable count from ~17 to ~10-12.
#
# Strategy: Instead of computing ALL exp arrays and gated variants
# upfront (Phase 0), then ALL dots (Phase 3-4), then ALL combines
# (Phase 5), we STAGE the computation per output:
#
#   Stage 1: Compute dv (needs exp_neg_g, k_neg, mask, b_a, b_dA)
#            -> write dv immediately, freeing b_a_masked, b_dv_*
#   Stage 2: Compute dq (needs exp_pos_g, q_pos, k_neg, b_dA)
#            -> write dq immediately, freeing exp_pos_g, b_dq_*
#   Stage 3: Compute dk (needs exp_neg_g, exp_gn_minus_g, q_pos, b_dA)
#            -> write dk immediately
#
# The key insight: the Mosaic compiler CAN schedule ops within a
# single kernel body to reduce register pressure if intermediate
# values have shorter live ranges. By writing outputs as soon as
# they're ready, we create natural "death points" for temporaries.
#
# Expected reduction: peak ~17 arrays -> ~12 arrays simultaneously
# live, potentially cutting register spills by 30-40%.
# ============================================================
...
def _chunk_gla_bwd_fused_kernel(
    ...
    *, BT, scale,
    ...
    # -------------------------------------------------------
    # Shared setup: causal mask and gating values needed by
    # multiple stages. Compute only what's needed upfront.
    # -------------------------------------------------------
    mask = jnp.arange(BT)[:, None] >= jnp.arange(BT)[None, :]

    # exp(-g) is needed for k_neg (used in A, dq_intra, dk combine)
    exp_neg_g = jnp.exp(-b_g)              # [BT, K]
    k_neg = (b_k * exp_neg_g).astype(b_k.dtype)  # [BT, K]: k * exp(-g)

    # exp(g) is needed for q_pos (used in A, dk_intra, dq combine)
    exp_pos_g = jnp.exp(b_g)               # [BT, K]
    q_pos = (b_q * exp_pos_g).astype(b_q.dtype)  # [BT, K]: q * exp(g)

    # -------------------------------------------------------
    # Recompute A and dA_raw (shared by dv and dk/dq stages)
    # These are [BT,BT] matrices; compute once and reuse.
    # -------------------------------------------------------
    ...
    # -------------------------------------------------------
    # STAGE 1: Compute and write dv
    # dv = A_masked.T @ do  +  k_decay @ dh
    # After writing, b_a_masked, b_dv_intra, b_dv_inter are dead.
    # -------------------------------------------------------
    b_dv_intra = jnp.dot(b_a_masked.T.astype(b_do.dtype), b_do,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)   # [BT, V]

    # Compute exp(gn - g) and k_decay only now (needed for dv_inter and dk)
    b_gn = b_g[BT - 1, :]  # [K]
    ...
    # Write dv immediately -- frees b_dv_intra, b_dv_inter, b_a_masked
    dv_ref[0, 0] = (b_dv_intra + b_dv_inter).astype(dv_ref.dtype)

    # -------------------------------------------------------
    # STAGE 2: Compute and write dq
    # dq = (dA @ k_neg) * exp(g) + (do @ h.T) * scale * exp(g)
    # After writing, b_dq_intra, b_dq_inter are dead.
    # -------------------------------------------------------
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    ...
    MUTATION (reduce_live_bwd):
      - A is recomputed inside the kernel (inherited from L1_reduce_inputs)
      - dg output is removed (inherited from skip_dg)
      - Computation is staged per-output to reduce peak live variables
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
