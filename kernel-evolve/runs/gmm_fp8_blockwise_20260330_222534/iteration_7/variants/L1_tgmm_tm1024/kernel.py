"""GMM block-wise FP8 quantization kernel for TPU -- L1_tgmm_tm1024 variant.

Cross-pollinates SO14 (tgmm TM=1024) with SO15 (bf16 scales) from the L1
code path. Parent is L1_scale_bf16 (iteration 6), which proved bf16 scale
dtype effective with phase-specialized tiling (256,512,256, 256,512,128,
2048,512,128).

This variant halves tgmm TM from 2048 to 1024. The tgmm kernel's M dimension
corresponds to the K dimension of the original matmul (K=2048 for Gate/Up,
K=512 for Down). TM=1024 may improve MXU utilization by reducing per-tile
work and increasing pipeline parallelism, while still being large enough to
amortize loop overhead vs TM=128.

Tiling vs parent (L1_scale_bf16):
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (1024, 512, 128) -- TM 2048->1024 (SO14)
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,128, 1024,512,128)
  - Remove min(t, tile_size) clamping to allow tiling > 128
  - scale_dtype=jnp.bfloat16 for both lhs and rhs quantization (SO15)
...
def gmm_fp8_blockwise(
    ...
    L1_tgmm_tm1024 variant: cross-pollinates SO14 (tgmm TM=1024) with SO15
    (bf16 scales). fwd (256,512,256), bwd_gmm (256,512,128), tgmm (1024,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (1024, 512, 128) -- TM=1024 (halved from 2048) to improve
    # MXU utilization via better pipeline parallelism while still amortizing
    # loop overhead. TK=512 reduces K-loop iterations. Since tgmm operates
    # in bf16 (not fp8), larger K tiles are safe.
