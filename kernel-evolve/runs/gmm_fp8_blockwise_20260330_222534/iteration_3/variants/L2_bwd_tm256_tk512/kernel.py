"""GMM block-wise FP8 quantization kernel for TPU -- bwd_tm256_tk512 variant.

Builds on L1_bwd_tm256 (2.290x) which proved TM=256 for forward doubles MXU
ops by matching per-group M exactly (M=8192/G=32=256). This variant combines
TM=256 with TK=512 for bwd_gmm -- halving K-loop iterations while maintaining
per-group alignment.
...
L1_bwd_tm256 used (256,256,128, 256,256,128, 2048,512,128). This variant
changes bwd_gmm TK from 256 to 512:
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,256,128, 256,512,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 256, 128, 256, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    bwd_tm256_tk512 variant: phase-specialized tiling with
    fwd (256,256,128), bwd_gmm (256,512,128), tgmm (2048,512,128).
