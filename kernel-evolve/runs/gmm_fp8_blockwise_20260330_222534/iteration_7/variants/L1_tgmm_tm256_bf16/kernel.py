"""GMM block-wise FP8 quantization kernel for TPU -- L1_tgmm_tm256_bf16 variant.

Builds on L1_scale_bf16 which proved scale_dtype=bfloat16 effective with
phase-specialized tiling (256,512,256, 256,512,128, 2048,512,128).

This variant changes tgmm TM from 2048 to 256, testing whether TM=256
(matching per-group M alignment, same as fwd/bwd) provides even more
simplification than TM=1024 or TM=2048. The per-group M alignment
(M=8192/G=32=256) was the key insight for fwd/bwd (SO9). TM=256 for tgmm
means each tile processes exactly one group's worth of data, eliminating
cross-group boundary handling within a tile and maximizing alignment with
the problem structure.

Tiling vs parent (L1_scale_bf16):
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (256, 512, 128) -- TM changed from 2048 to 256
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,128, 256,512,128)
  - Remove min(t, tile_size) clamping to allow tiling > 128
  - scale_dtype=jnp.bfloat16 for both lhs and rhs quantization (was float32)
  - tgmm TM=256 matches per-group M exactly (M=8192/G=32=256)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 512, 128, 256, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_tgmm_tm256_bf16 variant: tgmm TM=256 (matching per-group M alignment)
    with scale_dtype=bfloat16. fwd (256,512,256), bwd_gmm (256,512,128),
    tgmm (256,512,128). Zero backward quantization with mixed-precision bwd_gmm.
    """
    tile_size = 128
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Quantize lhs to fp8 for forward gmm -- scale_dtype=bfloat16 to reduce
    # scale tensor size by 2x and leverage native bf16 support on TPU v7x
    lhs = qpl.quantize(
        lhs_bf16,
        qt_rule.act_qtype,
        channelwise_axes=[0],
    ...
    # Quantize rhs to fp8 for forward gmm -- scale_dtype=bfloat16 to reduce
    # scale tensor size by 2x and leverage native bf16 support on TPU v7x
    rhs = qpl.quantize(
        rhs,
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (256, 512, 128) -- adopted from L2's winning config
    ...
    # tgmm tiling: (256, 512, 128) -- TM=256 matches per-group M exactly
    # (M=8192/G=32=256), so each tile processes exactly one group's data.
    # This eliminates cross-group boundary handling within a tile, maximizing
    # alignment with the problem structure (same insight as fwd/bwd SO9).
