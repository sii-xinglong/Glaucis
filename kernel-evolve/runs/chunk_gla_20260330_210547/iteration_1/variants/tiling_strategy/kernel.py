...
# ============================================================
# Backward: Split kernel 1 — dq + dv (tiling_strategy)
#
# Motivation: The fused baseline kernel loads 8 refs simultaneously
# causing 1.9M register spills. Split into two kernels to halve
# the live register set per kernel:
#   Kernel 1 (dq + dv): needs q, k, v, g, h, A, do, dh → dq, dv
#   Kernel 2 (dk + dg): needs q, k, g, h, A, do, dh → dk, dg
#
# Both kernels use the same 8-input signature but each writes only
# 2 outputs instead of 4, allowing the compiler to retire output
# registers sooner and pack MXU ops more densely.
# ============================================================


def _chunk_gla_bwd_dqdv_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, a_ref, do_ref, dh_ref,
    dq_ref, dv_ref,
    *, BT, scale,
):
    """Computes dq and dv gradients for one chunk.

    Loads: q[BT,K], k[BT,K], v[BT,V], g[BT,K], h[K,V],
           A[BT,BT], do[BT,V], dh[K,V]
    Writes: dq[BT,K], dv[BT,V]

    With only 2 outputs instead of 4, the compiler has 2 fewer
    live write-buffers to manage, reducing register pressure
    substantially. The key bottleneck intermediates (dA, k_neg,
    exp terms) are the same size but now compete with fewer outputs.
    """
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    ...
    # dA = do @ v.T * scale  (BT x BT, causal-masked)
    b_dA = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                   precision=jax.lax.Precision.HIGHEST,
                   preferred_element_type=jnp.float32) * scale
    b_dA = jnp.where(mask, b_dA, 0.0)

    # dv = A_masked.T @ do  (intra)  +  k_decay @ dh  (inter)
    ...
    # dq = dA @ k_neg * exp(g)  (intra)  +  do @ h.T * scale * exp(g)  (inter)
...
# ============================================================
# Backward: Split kernel 2 — dk + dg (tiling_strategy)
# ============================================================


def _chunk_gla_bwd_dkdg_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, a_ref, do_ref, dh_ref,
    dk_ref, dg_ref,
    *, BT, scale,
):
    """Computes dk and dg gradients for one chunk.

    Loads: q[BT,K], k[BT,K], v[BT,V], g[BT,K], h[K,V],
           A[BT,BT], do[BT,V], dh[K,V]
    Writes: dk[BT,K], dg[BT,K]

    With only 2 outputs, the compiler can retire output registers
    earlier. dg computation requires the dq/dk values computed here
    but does NOT require dv, so we recompute only dq_intra/dq_inter
    locally rather than communicating them across kernels.
    """
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    ...
    # Recompute dA (needed for dk and dg)
    b_dA = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                   precision=jax.lax.Precision.HIGHEST,
                   preferred_element_type=jnp.float32) * scale
    b_dA = jnp.where(mask, b_dA, 0.0)

    # dk = dA.T @ q_pos * exp(-g)  (intra)  +  v @ dh.T * exp(gn - g)  (inter)
    ...
    # dg: needs dq and dk — recompute dq locally (small cost)
    k_neg = (b_k * jnp.exp(-b_g)).astype(b_k.dtype)
    b_dq_intra = jnp.dot(b_dA.astype(k_neg.dtype), k_neg,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32) * jnp.exp(b_g)
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    """Launch split backward Pallas kernels (tiling_strategy variant).

    Instead of one fused kernel with 8 inputs + 4 outputs (causing
    1.9M register spills), we launch two kernels each with 8 inputs
    + 2 outputs. This halves the number of simultaneously live output
    register buffers per kernel, reducing register pressure and
    enabling the compiler to schedule MXU ops more efficiently.

    Kernel 1: dq + dv
    Kernel 2: dk + dg  (recomputes dA and dq locally — cost is
                         small vs. the spill/fill reduction)
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    dq, dv = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dqdv_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
    ...
    dk, dg = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dkdg_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
