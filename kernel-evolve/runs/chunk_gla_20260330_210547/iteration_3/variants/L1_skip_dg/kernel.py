...
#
# MUTATION: Skip dg computation entirely
#
# The caller discards dg: `dq, dk, dv, _ = chunk_gla_bwd_fused(...)`.
# This mutation removes ALL dg-related computation from the kernel
# and reduces outputs from 4 to 3.
#
# Savings:
#   1. Remove 1 MXU matmul: M_upper @ dg_raw ([64,64] @ [64,128])
#   2. Remove 1 DMA write: dg output tile (BT*K = 64*128 bfloat16)
#   3. Free M_upper intermediate: 64*64 float32 = 16KB register pressure
#   4. Free dg_raw, dgk_inter, dg_rev_cumsum intermediates
#   5. Remove VPU ops: exp(b_gn)*sum, q*dq-k*dk, mask_upper creation
#
# With 2.4M register spills in the base kernel, reducing live values
# by ~10% (M_upper 16KB + dg intermediates) should meaningfully
# reduce spill/fill overhead.
#
# Parent: L1_reduce_inputs (recomputes A inside kernel)
# ============================================================
...
def _chunk_gla_bwd_fused_kernel(
    ...
    *, BT, scale,
    ...
    exp_pos_g = jnp.exp(b_g)               # [BT, K] — used by dq_intra, A recompute
    exp_neg_g = jnp.exp(-b_g)              # [BT, K] — used by k_neg (dq), dk_intra, A recompute
    ...
    #
    # A recomputation eliminates one HBM read (the A tile)
    # and one Ref input slot, reducing register pressure.
    # q_pos and k_neg are already computed above for other uses.
    # -------------------------------------------------------
    ...
    #
    # MUTATION: dg computation entirely removed. The caller
    # discards dg anyway: dq, dk, dv, _ = chunk_gla_bwd_fused(...)
    # Savings: 1 MXU matmul (M_upper @ dg_raw), M_upper 16KB,
    # dgk_inter, dg_raw, dg_rev_cumsum intermediates, 1 DMA write.
    # -------------------------------------------------------
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    ...
    MUTATION: Skip dg computation entirely. The caller discards dg,
    so we eliminate it from the kernel outputs (4 -> 3), removing
    1 MXU matmul, 1 DMA write, and ~16KB of register pressure from
    the M_upper intermediate.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
