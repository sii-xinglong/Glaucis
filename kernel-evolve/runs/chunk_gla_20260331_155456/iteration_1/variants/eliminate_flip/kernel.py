...
# of [BT,K] exp arrays.
#
# MUTATION (eliminate_flip): Removes ALL jnp.flip() calls from
# the backward host function. Instead of flipping inputs and
# outputs (which creates ~288MB of HBM copies), we use reversed
# BlockSpec index_maps (NT-1-t) so the kernel reads chunks in
# reverse order directly from the original arrays. The kernel
# writes outputs to slot NT-1-i_t so they land in correct
# forward order, eliminating output flips too.
#
# Savings:
#   - Eliminates g_cumsum [B,T,H,K] from residuals (~67MB HBM)
#   - Eliminates 5 input jnp.flip() copies (~288MB HBM)
#   - Eliminates 3 output jnp.flip() copies (~172MB HBM)
#   - Eliminates chunking/reshaping of q/k/v/do
# ============================================================
...
def _chunk_gla_bwd_eliminate_gcumsum_kernel(
    ...
    *, BT, NT, scale,
    ...
    """Fused backward kernel with g_cumsum eliminated and reversed output writes.
    ...
    MUTATION (eliminate_flip): Input BlockSpecs use reversed index_maps
    (NT-1-t), so i_t=0 processes the LAST original chunk. Outputs are
    written to slot NT-1-i_t so they land in correct forward order
    without needing a post-hoc jnp.flip().

    At each step i_t (grid time dimension):
      1. Load current dh state from scratch_ref
      2. Compute dq/dk/dv using current dh
      3. Write outputs to dq_ref[0, NT-1-i_t, 0] (correct forward slot)
    ...
    # i.e. the END of the original sequence because inputs are read reversed)
    @pl.when(i_t == 0)
    def init():
        scratch_ref[:, :] = jnp.zeros((BK, BV), dtype=jnp.float32)
    ...
    # MUTATION (eliminate_flip): Write to NT-1-i_t so outputs land
    # in correct forward order without needing post-hoc jnp.flip()
    # MUTATION (eliminate_gcumsum): exp gating uses [BT] broadcast
    # -------------------------------------------------------
...
def chunk_gla_bwd_eliminate_gcumsum(q, k, v, h, do, g_gamma, scale, chunk_size):
    """Fused backward: single Pallas kernel with g_cumsum eliminated and flips removed.
...
#
# This saves ~67MB HBM by not storing g_cumsum [B,T,H,K] as a residual,
# and saves ~32KB bandwidth per backward chunk by not loading it.
#
# Total pallas_calls: 2 (unchanged from recompute_dh_v2)
# ============================================================
