"""GMM block-wise FP8 quantization kernel for TPU -- L1_tgmm_tk1024 variant.

Builds on L1_adopt_l2_tiling (2.651x) which proved phase-specialized tiling
with (256,512,128, 256,512,128, 2048,512,128) is effective.

This variant increases tgmm TK from 512 to 1024, doubling the K-tile size
in the tgmm pass. Since tgmm operates in bf16 (not fp8), larger K tiles
are safe and further reduce K-loop iterations. For K=8192 (M dimension
transposed), this means 8 iterations instead of 16.

Tiling changes vs parent:
  - Forward gmm:  (256, 512, 128) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (2048, 1024, 128) -- TK 512->1024
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,128, 256,512,128, 2048,1024,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 256, 512, 128, 2048, 1024, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_tgmm_tk1024 variant: tgmm TK 512->1024 to halve K-loop iterations.
    fwd (256,512,128), bwd_gmm (256,512,128), tgmm (2048,1024,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # TK=512 uses smaller K tiles to reduce register pressure in the
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (2048, 1024, 128) -- TK=1024 to further reduce K-loop
    # iterations. Since tgmm operates in bf16 (not fp8), larger K tiles are
    # safe and reduce loop overhead without fp8 quantization tile constraints.
