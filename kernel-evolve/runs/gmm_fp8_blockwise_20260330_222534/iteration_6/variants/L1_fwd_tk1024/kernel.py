"""GMM block-wise FP8 quantization kernel for TPU -- L1_fwd_tk1024 variant.
...
This variant increases forward TK from 512 to 1024, doubling the K-tile size
in the forward gmm pass. For Gate/Up (K=2048), this means 2 K-iterations
instead of 4. For Down (K=512), TK is clamped to 512 (since K < TK).

Tiling changes vs parent:
  - Forward gmm:  (256, 1024, 256) -- TK 512->1024
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,1024,256, 256,512,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 1024, 256, 256, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_fwd_tk1024 variant: forward TK 512->1024 to halve K-loop iterations.
    fwd (256,1024,256), bwd_gmm (256,512,128), tgmm (2048,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Forward gmm with phase-specialized tiling: (256, 1024, 256)
    # TM=256 matches per-group M exactly (M=8192 / G=32 = 256)
    # TK=1024 reduces K-loop iterations (K=2048: 2 iters instead of 4)
