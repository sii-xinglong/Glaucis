# `pallas-evolve:init-kernel` Skill Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a new `pallas-evolve:init-kernel` skill that automates initialization of kernel optimization projects from the `primatrix/pallas-kernel` upstream repository.

**Architecture:** The skill is a single `SKILL.md` file containing procedural instructions for Claude Code. When invoked, Claude follows these instructions to clone the upstream repo, consolidate multi-file kernels into self-contained files, generate template/ref/test/config files, validate, and clean up. A supporting pytest file validates the output conventions the skill must produce.

**Tech Stack:** Markdown (SKILL.md), Python (tests), YAML (config generation), Git (clone upstream)

**Spec:** `docs/superpowers/specs/2026-03-31-init-kernel-skill-design.md`

---

## Chunk 1: Convention validation tests

These tests verify that files following the init-kernel output conventions work correctly with the existing evaluate.py function discovery, config validation, and mutation extraction systems. Writing these first ensures we have a safety net before writing the skill.

### Task 1: Test that generated ref files are discoverable by evaluate.py

**Files:**
- Create: `kernel-evolve/tests/test_init_kernel_conventions.py`

- [ ] **Step 1: Write tests for ref file conventions**

Write tests that verify evaluate.py's `_resolve_compute_fn` can discover functions from a ref file following the init-kernel output pattern:
...
Run: `cd kernel-evolve && python -m pytest tests/test_init_kernel_conventions.py::test_ref_has_valid_syntax tests/test_init_kernel_conventions.py::test_ref_simple_compute_discoverable tests/test_init_kernel_conventions.py::test_ref_reference_fn_discoverable tests/test_init_kernel_conventions.py::test_ref_has_source_traceability -v`
Expected: All 4 PASS

- [ ] **Step 3: Commit**

```bash
git add kernel-evolve/tests/test_init_kernel_conventions.py
git commit -m "test(init-kernel): add ref file convention tests"
```

### Task 2: Test that generated template files work with mutation extraction and evaluate.py

**Files:**
- Modify: `kernel-evolve/tests/test_init_kernel_conventions.py`

- [ ] **Step 1: Add template convention tests**

Append to `test_init_kernel_conventions.py`:
...
Run: `cd kernel-evolve && python -m pytest tests/test_init_kernel_conventions.py -k "template" -v`
Expected: All 5 PASS
...
```bash
git add kernel-evolve/tests/test_init_kernel_conventions.py
git commit -m "test(init-kernel): add template file convention tests"
```

### Task 3: Test that generated YAML config validates against Pydantic schema

**Files:**
- Modify: `kernel-evolve/tests/test_init_kernel_conventions.py`

- [ ] **Step 1: Add config convention tests**
...
Run: `cd kernel-evolve && python -m pytest tests/test_init_kernel_conventions.py -k "config" -v`
Expected: All 3 PASS
...
```bash
git add kernel-evolve/tests/test_init_kernel_conventions.py
git commit -m "test(init-kernel): add YAML config convention tests"
```

### Task 4: Test standalone test file conventions

**Files:**
- Modify: `kernel-evolve/tests/test_init_kernel_conventions.py`

- [ ] **Step 1: Add standalone test convention tests**
...
Run: `cd kernel-evolve && python -m pytest tests/test_init_kernel_conventions.py -v`
...
```bash
git add kernel-evolve/tests/test_init_kernel_conventions.py
git commit -m "test(init-kernel): add standalone test convention tests"
```

---

## Chunk 2: SKILL.md — Frontmatter, Arguments, Clone & Discovery

The SKILL.md is the core deliverable. It contains the procedural instructions Claude Code follows when the user invokes `/pallas-evolve:init-kernel`. We write it in stages, building up from the argument parsing through to the cleanup.

### Task 5: Create SKILL.md with frontmatter and argument parsing

**Files:**
- Create: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Create skill directory**

```bash
mkdir -p kernel-evolve/plugins/pallas-evolve/skills/init-kernel
```

- [ ] **Step 2: Write the SKILL.md frontmatter and argument parsing section**

Create `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md` with:
...
git commit -m "feat(init-kernel): create SKILL.md with frontmatter and arg parsing"
```

### Task 6: Add Step 1 (Clone) and Step 2 (Discover) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append clone and discovery steps**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add clone and discovery steps"
```

### Task 7: Add Step 2.5 (Analyze kernel structure) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append kernel analysis step**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add kernel structure analysis step"
```

---

## Chunk 3: SKILL.md — File Generation Steps

### Task 8: Add Step 3 (Generate reference kernel) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append reference kernel generation step**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add reference kernel generation step"
```

### Task 9: Add Step 4 (Generate template kernel) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append template kernel generation step**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add template kernel generation step"
```

### Task 10: Add Steps 5-6 (Copy tests + Generate comparison test) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append test generation steps**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add test copy and comparison test generation steps"
```

---

## Chunk 4: SKILL.md — Config, Validation, Cleanup

### Task 11: Add Steps 7-8 (YAML config + validation) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append config generation and validation steps**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add YAML config generation and validation steps"
```

### Task 12: Add Steps 9-10 (Verification + Cleanup) to SKILL.md

**Files:**
- Modify: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`

- [ ] **Step 1: Append verification and cleanup steps**
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md
git commit -m "feat(init-kernel): add verification and cleanup steps"
```

### Task 13: Final review — read through complete SKILL.md and verify consistency

**Files:**
- Read: `kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md`
- Read: `kernel-evolve/plugins/pallas-evolve/skills/start/SKILL.md` (for style comparison)

- [ ] **Step 1: Read the complete SKILL.md and compare with existing skills**

Read the full SKILL.md and verify:
1. Frontmatter format matches other skills (`name`, `description`)
2. Step numbering is consistent and complete (Steps 1 through 10)
3. All placeholders (`{KERNEL_NAME}`, `{BRANCH}`, etc.) are used consistently
4. Error handling at each step references Step 10 cleanup
5. The output file paths match the spec
6. The function names (`simple_compute`, `reference_fn`, `optimized_compute`) match evaluate.py conventions
7. EVOLVE-BLOCK placement rules are clear and match existing patterns

- [ ] **Step 2: Fix any inconsistencies found**

If any issues are found, edit the SKILL.md to fix them.

- [ ] **Step 3: Run all tests**

```bash
cd kernel-evolve && python -m pytest tests/test_init_kernel_conventions.py -v
```
...
```bash
git add kernel-evolve/plugins/pallas-evolve/skills/init-kernel/SKILL.md kernel-evolve/tests/test_init_kernel_conventions.py
git commit -m "feat(init-kernel): complete pallas-evolve:init-kernel skill

New skill that automates kernel initialization from primatrix/pallas-kernel:
- Clones upstream repo and consolidates multi-file kernels
- Generates self-contained ref and template files
- Creates pytest and standalone TPU tests
- Generates YAML config validated against Pydantic schema
- Preserves upstream source for traceability"
```
