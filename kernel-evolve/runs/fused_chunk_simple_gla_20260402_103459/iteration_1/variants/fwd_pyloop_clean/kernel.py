...
def _fused_chunk_fwd_4step_kernel(
    ...
    *,
    ...
    # --- Process all 4 sub-steps via a Python for-loop (JAX trace-time unrolling) ---
    for step in range(4):
        b_q = q_ref[0, 0, pl.ds(step * BT, BT), :]
        b_k = k_ref[0, 0, pl.ds(step * BT, BT), :]
        b_v = v_ref[0, 0, pl.ds(step * BT, BT), :]
        b_h = scratch_ref[...]
        ...
        if g_ref is not None:
            b_g = g_ref[0, 0, pl.ds(step * BT, BT), 0].astype(jnp.float32)
        ...
        o_ref[0, 0, 0, pl.ds(step * BT, BT), :] = partial.astype(o_ref.dtype)
