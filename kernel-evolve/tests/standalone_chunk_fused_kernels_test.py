"""Standalone integration test for chunk_fused_kernels kernel.

Run on TPU:
    python tests/standalone_chunk_fused_kernels_test.py

Verifies:
  1. Both template and reference produce finite results
  2. Template matches reference within atol=1.0
...
def main():
    ...
    tmpl_ns = {}
    ref_ns = {}
    exec(open("examples/kernels/chunk_fused_kernels.py").read(), tmpl_ns)
    exec(open("examples/kernels/chunk_fused_kernels_ref.py").read(), ref_ns)

    shapes = {"B": 2, "T": 256, "H": 4, "K": 128, "V": 128, "chunk_size": 64}
    print(f"\nShapes: {shapes}")
