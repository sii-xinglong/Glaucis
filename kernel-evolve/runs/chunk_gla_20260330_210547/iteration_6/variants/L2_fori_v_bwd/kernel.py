...
#
# MUTATION (fori_v_bwd): Use jax.lax.fori_loop to split backward
# computation into two sequential passes, hiding one pass's live
# arrays from the compiler when analyzing the other pass.
#
# Profile motivation (FP23): The backward fused kernel
# _chunk_gla_bwd_fused_kernel has 2.5M register spills. All ~24
# intermediate arrays are simultaneously live because the compiler
# sees the entire kernel body. fori_loop hides iterations from
# the compiler (it only sees one body at a time), so each pass's
# arrays become dead before the next pass allocates registers.
#
# Split strategy:
#   Pass 0 (i == 0): Compute dv
#     Needs: b_a_masked, b_dA, k_decay, b_dh, b_do (for dv terms)
#     Writes: dv_ref
#     After this pass, b_a_masked / b_do / k_decay can be released.
#
#   Pass 1 (i == 1): Compute dq, dk
#     Needs: b_dA, q_pos, k_neg, b_h, b_dh, b_do, exp_pos_g, exp_neg_g,
#            exp_gn_minus_g (for dq/dk terms)
#     Writes: dq_ref, dk_ref
#
# Arrays that only live within one pass no longer need to coexist
# with arrays from the other pass, halving peak register pressure.
#
# Implementation: The fori_loop body uses lax.cond on the loop index
# to select pass 0 vs pass 1. Pre-computed shared arrays (b_dA,
# b_a_masked, q_pos, k_neg, k_decay, exp_pos_g, exp_neg_g,
# exp_gn_minus_g) are placed in the carry so both passes can access
# them, but the key insight is that dv-specific intermediates
# (b_dv_intra, b_dv_inter) are computed and consumed within pass 0,
# and dq/dk-specific intermediates within pass 1, preventing them
# from being simultaneously live.
# ============================================================
...
def _chunk_gla_bwd_fused_kernel(
    ...
    *, BT, scale,
    ...
    # These are shared between both fori_loop passes.
    # -------------------------------------------------------
    ...
    # -------------------------------------------------------
    # Phase 3 + 4: Use fori_loop over 2 passes to reduce
    # peak register pressure. The compiler sees only one pass
    # body at a time (FP23 key insight).
    #
    # Pass 0: Compute and write dv
    # Pass 1: Compute and write dq, dk
    #
    # Carry = (b_dA, b_a_masked, q_pos, k_neg, k_decay,
    #          exp_pos_g, exp_neg_g, exp_gn_minus_g,
    #          b_h, b_dh, b_do)
    # All shared arrays travel through the carry so the compiler
    # knows they are live across iterations (this is intentional:
    # the savings come from dv-specific vs dq/dk-specific arrays
    # NOT being simultaneously live).
    # -------------------------------------------------------

    def bwd_body(i, carry):
        (b_dA_, b_a_masked_, q_pos_, k_neg_, k_decay_,
         exp_pos_g_, exp_neg_g_, exp_gn_minus_g_,
         b_h_, b_dh_, b_do_) = carry

        def pass0(_):
            # Pass 0: Compute dv only
            # b_dv_intra: [BT,BT].T @ [BT,V] = [BT,V]
            b_dv_intra = jnp.dot(
                b_a_masked_.T.astype(b_do_.dtype), b_do_,
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )
            # b_dv_inter: [BT,K] @ [K,V] = [BT,V]
            b_dv_inter = jnp.dot(
                k_decay_, b_dh_.astype(k_decay_.dtype),
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )
        ...
        def pass1(_):
            # Pass 1: Compute dq and dk
            # dq_inter: [BT,V] @ [V,K] = [BT,K]  (via b_h.T)
            b_dq_inter = jnp.dot(
                b_do_.astype(b_h_.dtype), b_h_.astype(b_do_.dtype).T,
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )
            # dk_inter: [BT,V] @ [V,K] = [BT,K]  (via b_dh.T)
            b_dk_inter = jnp.dot(
                b_v.astype(b_dh_.dtype), b_dh_.astype(b_v.dtype).T,
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )
            # dq_intra: [BT,BT] @ [BT,K] = [BT,K]
            b_dq_intra_raw = jnp.dot(
                b_dA_.astype(k_neg_.dtype), k_neg_,
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )
            # dk_intra: [BT,BT].T @ [BT,K] = [BT,K]
            b_dk_intra_raw = jnp.dot(
                b_dA_.T.astype(q_pos_.dtype), q_pos_,
                precision=jax.lax.Precision.HIGHEST,
                preferred_element_type=jnp.float32,
            )

            b_dq = b_dq_intra_raw * exp_pos_g_ + b_dq_inter * (scale * exp_pos_g_)
            dq_ref[0, 0] = b_dq.astype(dq_ref.dtype)

            b_dk = b_dk_intra_raw * exp_neg_g_ + b_dk_inter * exp_gn_minus_g_
            dk_ref[0, 0] = b_dk.astype(dk_ref.dtype)

        lax.cond(i == 0, pass0, pass1, operand=None)
        return carry

    init_carry = (
        b_dA, b_a_masked, q_pos, k_neg, k_decay,
        exp_pos_g, exp_neg_g, exp_gn_minus_g,
        b_h, b_dh, b_do,
    )
    lax.fori_loop(0, 2, bwd_body, init_carry)
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    ...
    MUTATION (fori_v_bwd):
      ...
      - Backward computation split into 2 fori_loop passes to reduce
        peak register pressure (FP23: fori_loop hides iterations from
        the compiler, only one pass's arrays are live at a time)
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
