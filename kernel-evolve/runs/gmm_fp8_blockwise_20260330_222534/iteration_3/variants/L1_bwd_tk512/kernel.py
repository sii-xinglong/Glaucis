"""GMM block-wise FP8 quantization kernel for TPU -- bwd_tk512 variant.

Builds on L1_fwd_tk512 (2.335x) which showed forward TK=512 gave +1.8%
by halving K-loop iterations. This variant applies the same principle to
bwd_gmm by changing its TK from 256 to 512.
...
bwd_gmm computes dlhs = grad @ rhs^T. For this operation:
  Gate/Up bwd: grad [256, 512] @ rhs^T [512, 2048] -> K_eff=512, TK=512 = 1 iter
  Down bwd:    grad [256, 2048] @ rhs^T [2048, 512] -> K_eff=2048, TK=512 = 4 iters

With TK=256, Gate/Up bwd had 2 iterations, Down bwd had 8 iterations.
TK=512 halves both: Gate/Up to 1 iteration, Down to 4 iterations.

Phase-specialized tiling: (256,512,128, 1024,512,128, 2048,512,128)
  - Forward gmm:  (256, 512, 128) -- from L1_fwd_tk512 (proven +1.8%)
  - Backward gmm: (1024, 512, 128) -- TK=512 (was 256), halves K iterations
  - tgmm:         (2048, 512, 128) -- unchanged from L1_fwd_tk512
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 1024, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    bwd_tk512 variant: phase-specialized tiling with
    fwd (256,512,128), bwd_gmm (1024,512,128), tgmm (2048,512,128).
    bwd_gmm TK increased from 256 to 512 to halve K-loop iterations.
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Forward gmm with phase-specialized tiling: (256, 512, 128)
    # TM=256 matches per-group M exactly (M=8192 / G=32 = 256)
    # TK=512 halves K-loop iterations vs TK=256 (proven +1.8% in L1_fwd_tk512)
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (1024, 512, 128) -- TK=512 halves K-loop iterations
    # vs previous TK=256. For Gate/Up bwd (K_eff=512): 1 iter vs 2 iters.
    # For Down bwd (K_eff=2048): 4 iters vs 8 iters.
