...
#
# MUTATION (L2_reduce_intermediates): Restructure backward fused kernel
# to reduce peak live intermediates and register pressure.
#
# Root cause: The base kernel pre-computes ALL exp values, gated variants,
# and matmul results before writing any output. This keeps ~20+ arrays
# simultaneously live, causing 2.5M register spills.
#
# Strategy: Compute-and-consume in 3 tighter scopes (dv → dq → dk).
# Each scope recomputes the shared intermediates it needs (exp_pos_g,
# exp_neg_g, q_pos, k_neg, A, dA), writes its output gradient, then
# lets those intermediates go out of scope. The compiler sees real WRITE
# barriers between scopes, so it can free arrays that are genuinely no
# longer needed.
#
# Cost: A matrix recomputed 2x (once for dv/dq, once for dk via dA.T),
# dA recomputed once. This trades ~2 extra BT×BK matmuls for a potential
# reduction from ~20 peak live arrays to ~8-10 per scope.
#
# Peak live per scope:
#   Scope 1 (dv): b_g, b_gn, exp_pos_g, exp_neg_g, exp_gn_minus_g,
#                 q_pos, k_neg, k_decay, b_a, mask, b_a_masked,
#                 b_dv_intra, b_dv_inter → write dv → free all
#   Scope 2 (dq): exp_pos_g, exp_neg_g, q_pos, k_neg (recomputed),
#                 b_dA_raw, b_dA, b_dq_intra_raw, b_dq_inter → write dq → free
#   Scope 3 (dk): exp_neg_g, exp_gn_minus_g, q_pos, k_neg (recomputed),
#                 b_dA (recomputed), b_dk_intra_raw, b_dk_inter → write dk → free
# ============================================================
...
def _chunk_gla_bwd_fused_kernel(
    ...
    *, BT, scale,
    ...
    # Load shared inputs (these are load-once; they stay live across all scopes
    # but are cheap scalars/small refs, not large intermediate arrays)
    b_q = q_ref[0, 0]    # [BT, K]
    b_k = k_ref[0, 0]    # [BT, K]
    b_v = v_ref[0, 0]    # [BT, V]
    ...
    b_gn = b_g[BT - 1, :]  # [K]: last-row gate values

    # Causal mask (constant; cheap to compute, referenced in all scopes)
    mask = jnp.arange(BT)[:, None] >= jnp.arange(BT)[None, :]

    # ===========================================================
    # Scope 1: Compute and write dv
    # Peak live intermediates: ~13 arrays (down from ~20+ baseline)
    # ===========================================================

    # Compute gating values needed for dv
    exp_pos_g_1 = jnp.exp(b_g)                          # [BT, K]
    exp_neg_g_1 = jnp.exp(-b_g)                         # [BT, K]
    exp_gn_minus_g_1 = jnp.exp(b_gn[None, :] - b_g)    # [BT, K]

    # Gated key variants for dv
    k_neg_1 = (b_k * exp_neg_g_1).astype(b_k.dtype)         # [BT, K]: k*exp(-g)
    k_decay_1 = (b_k * exp_gn_minus_g_1).astype(b_k.dtype)  # [BT, K]: k*exp(gn-g)
    q_pos_1 = (b_q * exp_pos_g_1).astype(b_q.dtype)         # [BT, K]: q*exp(g)

    # Recompute A = q_pos @ k_neg.T * scale  [BT, BT]
    b_a_1 = jnp.dot(q_pos_1, k_neg_1.T,
                    precision=jax.lax.Precision.HIGHEST,
                    preferred_element_type=jnp.float32) * scale

    # Apply causal mask to A
    b_a_masked_1 = jnp.where(mask, b_a_1, 0.0)   # [BT, BT]

    # dv_intra = A_masked.T @ do  [BT, V]
    b_dv_intra = jnp.dot(b_a_masked_1.T.astype(b_do.dtype), b_do,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # dv_inter = k_decay @ dh  [BT, V]
    b_dv_inter = jnp.dot(k_decay_1, b_dh.astype(b_k.dtype),
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # WRITE dv — hard barrier: compiler can free all scope-1 intermediates
    dv_ref[0, 0] = (b_dv_intra + b_dv_inter).astype(dv_ref.dtype)

    # ===========================================================
    # Scope 2: Compute and write dq
    # Recompute exp_pos_g, q_pos, k_neg, A, dA (not kept from scope 1)
    # Peak live intermediates: ~10 arrays
    # ===========================================================

    # Recompute gating and gated variants for dq
    exp_pos_g_2 = jnp.exp(b_g)           # [BT, K]
    exp_neg_g_2 = jnp.exp(-b_g)          # [BT, K]
    k_neg_2 = (b_k * exp_neg_g_2).astype(b_k.dtype)  # [BT, K]

    # Recompute A for dA (needed as dA = do @ v.T * scale, masked)
    q_pos_2 = (b_q * exp_pos_g_2).astype(b_q.dtype)  # [BT, K]

    # dA_raw = do @ v.T * scale  [BT, BT]
    b_dA_raw_2 = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32) * scale

    # Apply causal mask to dA
    b_dA_2 = jnp.where(mask, b_dA_raw_2, 0.0)   # [BT, BT]

    # dq_intra = dA @ k_neg  [BT, K]
    b_dq_intra = jnp.dot(b_dA_2.astype(k_neg_2.dtype), k_neg_2,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # dq_inter = do @ h.T  [BT, K]
    b_dq_inter = jnp.dot(b_do, b_h.astype(b_do.dtype).T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # Combine: dq = (dq_intra + dq_inter * scale) * exp(g)
    b_dq = b_dq_intra * exp_pos_g_2 + b_dq_inter * (scale * exp_pos_g_2)

    # WRITE dq — hard barrier: compiler can free all scope-2 intermediates
    dq_ref[0, 0] = b_dq.astype(dq_ref.dtype)

    # ===========================================================
    # Scope 3: Compute and write dk
    # Recompute exp_neg_g, exp_gn_minus_g, q_pos, k_neg, dA
    # Peak live intermediates: ~10 arrays
    # ===========================================================

    # Recompute gating values needed for dk
    exp_neg_g_3 = jnp.exp(-b_g)                        # [BT, K]
    exp_gn_minus_g_3 = jnp.exp(b_gn[None, :] - b_g)   # [BT, K]
    q_pos_3 = (b_q * jnp.exp(b_g)).astype(b_q.dtype)  # [BT, K]

    # Recompute dA (needed for dk_intra = dA.T @ q_pos)
    b_dA_raw_3 = jnp.dot(b_do.astype(b_v.dtype), b_v.T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32) * scale
    b_dA_3 = jnp.where(mask, b_dA_raw_3, 0.0)   # [BT, BT]

    # dk_intra = dA.T @ q_pos  [BT, K]
    b_dk_intra = jnp.dot(b_dA_3.T.astype(q_pos_3.dtype), q_pos_3,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # dk_inter = v @ dh.T  [BT, K]
    b_dk_inter = jnp.dot(b_v, b_dh.astype(b_v.dtype).T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)

    # Combine: dk = dk_intra * exp(-g) + dk_inter * exp(gn-g)
    b_dk = b_dk_intra * exp_neg_g_3 + b_dk_inter * exp_gn_minus_g_3

    # WRITE dk — hard barrier: compiler can free all scope-3 intermediates
