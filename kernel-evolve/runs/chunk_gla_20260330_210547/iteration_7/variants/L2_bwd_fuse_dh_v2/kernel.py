...
#
# MUTATION (bwd_fuse_dh_v2): Fix for L2_bwd_fuse_dh which failed with:
#   "Block shape (= (Blocked(1), Blocked(1), Blocked(128), Blocked(128)))
#    must have the same number of dimensions as the array shape
#    (2, 16, 64, 128, 128)"
#
# Root cause: h is 5D [B, H, NT, K, V] but the previous BlockSpec was
# 4D (1, 1, K, V), creating a rank mismatch.
#
# Fix: Use 5D BlockSpec (1, 1, 1, BK, BV) matching the 5D h array,
# with h_map returning all 5 indices: (b, h, t, 0, 0).
# The block shape (1, 1, 1, BK, BV) slices one (b, h, t) entry of
# the [B, H, NT, K, V] tensor, giving a (1, 1, 1, BK, BV) block.
# Inside the kernel, h_ref[0, 0, 0] reads the [BK, BV] matrix.
#
# Architecture (identical to L2_bwd_fuse_dh intent):
#   - Grid: (B, H, K/BK, V/BV, NT) with "arbitrary" time
#   - Flip all inputs along NT axis before pallas_call
#   - VMEM scratch holds running dh state [BK, BV]
#   - At each reverse step:
#       1. Use current dh (scratch) to compute dq/dk/dv
#       2. Update dh: dh = dh * exp(g_gamma*BT) + q_hat.T @ do
#   - Flip dq/dk/dv outputs back after call
#
# Eliminates:
#   - 1 kernel launch (chunk_bwd_dh_pallas)
#   - 256MB HBM write + 256MB HBM read for dh tensor (~512MB total)
#
# Profile motivation: bwd_fuse_dh is Priority 1 in profile_brief.md.
# ============================================================


def _chunk_gla_bwd_fuse_dh_v2_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, do_ref, g_gamma,
    dq_ref, dk_ref, dv_ref, scratch_ref,
    *, BT, NT, scale,
):
    """Fused backward kernel: dh VMEM accumulation + dq/dk/dv per chunk.

    Grid: (B, H, K_tiles, V_tiles, NT) with time as "arbitrary".
    h_ref block shape: (1, 1, 1, BK, BV) matching 5D h array [B, H, NT, K, V].
    Access: h_ref[0, 0, 0] gives the [BK, BV] matrix for this (b, h, t).

    At step i_t (reverse order, inputs pre-flipped):
      - scratch_ref holds dh flowing into the ORIGINAL chunk (NT-1-i_t)
      - Compute dq/dk/dv using current dh from scratch
      - Update: dh = dh * exp(g_gamma * BT) + q_hat.T @ do
    """
    BK = q_ref.shape[3]
    BV = do_ref.shape[3]
    i_b, i_h, i_k, i_v, i_t = (
    ...
    # Initialize dh scratch to zeros at t=0 (first step of REVERSE scan =
    # end of original sequence, so dh boundary = 0)
    @pl.when(i_t == 0)
    def init():
        scratch_ref[:, :] = jnp.zeros((BK, BV), dtype=jnp.float32)

    # Load inputs for this (reversed) time step
    # q, k, v, g, do all have 4D block shape (1, 1, BT, dim)
    b_q = q_ref[0, 0]    # [BT, BK]
    b_k = k_ref[0, 0]    # [BT, BK]
    b_v = v_ref[0, 0]    # [BT, BV]
    ...
    # h has 5D block shape (1, 1, 1, BK, BV); access as h_ref[0, 0, 0]
    b_h = h_ref[0, 0, 0].astype(jnp.float32)   # [BK, BV]

    # Current dh state (accumulated so far in reverse scan)
    b_dh = scratch_ref[...]    # [BK, BV]

    b_gn = b_g[BT - 1, :]  # last row of g_cumsum: [BK]

    # -------------------------------------------------------
    # Phase 0 (VPU): Pre-compute all exp/gate values
    # -------------------------------------------------------
    exp_pos_g = jnp.exp(b_g)                           # [BT, BK]
    exp_neg_g = jnp.exp(-b_g)                          # [BT, BK]
    exp_gn_minus_g = jnp.exp(b_gn[None, :] - b_g)     # [BT, BK]

    k_neg = (b_k * exp_neg_g).astype(b_k.dtype)           # [BT, BK]: k * exp(-g)
    k_decay = (b_k * exp_gn_minus_g).astype(b_k.dtype)    # [BT, BK]: k * exp(gn-g)
    q_pos = (b_q * exp_pos_g).astype(b_q.dtype)           # [BT, BK]: q * exp(g)
    ...
    # -------------------------------------------------------
    # Phase 5 (VPU): Combine and write outputs
    # -------------------------------------------------------
    ...
    # -------------------------------------------------------
    # Phase 6: Update dh state for the next reverse step
    # dh = dh * exp(g_gamma * BT) + (q * exp(g_ramp) * scale).T @ do
    # -------------------------------------------------------
    scratch_ref[...] *= exp(b_g_last)
...
def chunk_gla_bwd_fuse_dh_v2(q, k, v, g_cumsum, h, do, g_gamma, scale, chunk_size):
    """Fused backward: dh propagation + dq/dk/dv in a single pallas_call.

    MUTATION (bwd_fuse_dh_v2): Fixes L2_bwd_fuse_dh BlockSpec rank mismatch.

    The h input is 5D [B, NT, H, K, V]. The previous implementation used a
    4D BlockSpec (1, 1, K, V) which caused:
      "Block shape must have the same number of dimensions as the array shape"

    Fix: Use 5D BlockSpec (1, 1, 1, BK, BV) with h_map returning 5 indices.
    Inside the kernel, access h as h_ref[0, 0, 0] to get the [BK, BV] slice.

    Implementation:
      1. Reshape q/k/v/g/do to [B, H, NT, BT, dim], flip along NT axis
      2. Flatten back to [B, H, NT*BT, dim] for 4D BlockSpec compatibility
      3. Keep h as 5D [B, H, NT, K, V] with 5D BlockSpec (1, 1, 1, BK, BV)
      4. Run pallas_call with grid (B, H, 1, 1, NT) and arbitrary time
      5. VMEM scratch holds dh; compute dq/dk/dv then update dh each step
      6. Flip dq/dk/dv outputs back and reshape to [B, T, H, dim]
    """
    BK, BV, BT = 128, 128, chunk_size
    B, T, H, K = q.shape
    V = v.shape[-1]
    ...
    h_t = jnp.transpose(h, (0, 2, 1, 3, 4))   # [B, H, NT, K, V]

    # REVERSE the chunk order along NT axis (axis=2) for reverse scan
    ...
    h_flipped = jnp.flip(h_t, axis=2)          # [B, H, NT, K, V]

    # Flatten NT*BT for 4D BlockSpec compatibility (q/k/v/g/do)
    q_flat = q_flipped.reshape(B, H, NT * BT, K)   # [B, H, NT*BT, K]
    ...
    # h stays 5D: [B, H, NT, K, V] — used with 5D BlockSpec
    h_5d = h_flipped    # [B, H, NT, K, V]

    grid = (B, H, pl.cdiv(K, BK), pl.cdiv(V, BV), NT)

    # 4D index maps for q/k/v/g/do (flat [B, H, NT*BT, dim] arrays)
    def q_map(b, h, ki, vi, t):   return b, h, t, ki
    def k_map(b, h, ki, vi, t):   return b, h, t, ki
    def v_map(b, h, ki, vi, t):   return b, h, t, vi
    ...
    # 5D index map for h [B, H, NT, K, V] — FIX: BlockSpec ndim == array ndim
    # Block (1, 1, 1, BK, BV) selects one (b, h, t) chunk's K×V matrix.
    # ki and vi are always 0 here since BK=K and BV=V (single tile).
    def h_map(b, h, ki, vi, t):   return b, h, t, ki, vi

    # Output index maps for flat [B, H, NT*BT, dim] outputs
    def out_k_map(b, h, ki, vi, t): return b, h, t, ki
    def out_v_map(b, h, ki, vi, t): return b, h, t, vi

    dq_flat, dk_flat, dv_flat = pl.pallas_call(
        functools.partial(
            _chunk_gla_bwd_fuse_dh_v2_kernel,
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            grid=grid,
            in_specs=[
                pl.BlockSpec((1, 1, BT, BK), q_map),       # q: [B,H,NT*BT,K]
                pl.BlockSpec((1, 1, BT, BK), k_map),       # k: [B,H,NT*BT,K]
                pl.BlockSpec((1, 1, BT, BV), v_map),       # v: [B,H,NT*BT,V]
                pl.BlockSpec((1, 1, BT, BK), g_map),       # g: [B,H,NT*BT,K]
                # FIX: 5D BlockSpec (1,1,1,BK,BV) for 5D h [B,H,NT,K,V]
                pl.BlockSpec((1, 1, 1, BK, BV), h_map),   # h: [B,H,NT,K,V]
                pl.BlockSpec((1, 1, BT, BV), do_map),     # do: [B,H,NT*BT,V]
                pl.BlockSpec(memory_space=pltpu.SMEM),     # g_gamma: [H]
            ],
            out_specs=[
                pl.BlockSpec((1, 1, BT, BK), out_k_map),  # dq: [B,H,NT*BT,K]
                pl.BlockSpec((1, 1, BT, BK), out_k_map),  # dk: [B,H,NT*BT,K]
                pl.BlockSpec((1, 1, BT, BV), out_v_map),  # dv: [B,H,NT*BT,V]
            ],
            scratch_shapes=[pltpu.VMEM((BK, BV), jnp.float32)],
        ),
        out_shape=[
            jax.ShapeDtypeStruct((B, H, NT * BT, K), q.dtype),
            jax.ShapeDtypeStruct((B, H, NT * BT, K), k.dtype),
            jax.ShapeDtypeStruct((B, H, NT * BT, V), v.dtype),
        ],
        compiler_params=pltpu.CompilerParams(
            dimension_semantics=("parallel", "parallel", "arbitrary", "arbitrary", "arbitrary"),
            disable_bounds_checks=True,
        ),
    )(q_flat, k_flat, v_flat, g_flat, h_5d, do_flat, g_gamma)

    # Outputs are in reversed chunk order; flip back to original time order
    dq_chunked = dq_flat.reshape(B, H, NT, BT, K)
    dk_chunked = dk_flat.reshape(B, H, NT, BT, K)
    dv_chunked = dv_flat.reshape(B, H, NT, BT, V)

    dq_ordered = jnp.flip(dq_chunked, axis=2)   # [B, H, NT, BT, K]
    dk_ordered = jnp.flip(dk_chunked, axis=2)   # [B, H, NT, BT, K]
    dv_ordered = jnp.flip(dv_chunked, axis=2)   # [B, H, NT, BT, V]

    # Collapse NT*BT and transpose back to [B, T, H, dim]
    dq_t = dq_ordered.reshape(B, H, T, K)
    dk_t = dk_ordered.reshape(B, H, T, K)
    dv_t = dv_ordered.reshape(B, H, T, V)

    dq = jnp.transpose(dq_t, (0, 2, 1, 3))   # [B, T, H, K]
    dk = jnp.transpose(dk_t, (0, 2, 1, 3))   # [B, T, H, K]
    dv = jnp.transpose(dv_t, (0, 2, 1, 3))   # [B, T, H, V]
...
# custom_vjp wrapper
#
# MUTATION (bwd_fuse_dh_v2 + fuse_fwd_combined):
#   _fwd: uses chunk_fwd_combined (single pallas_call forward)
#   _bwd: uses chunk_gla_bwd_fuse_dh_v2 (single pallas_call backward,
#         fixing the 5D BlockSpec rank mismatch from L2_bwd_fuse_dh)
#
# Total pallas_calls: 2 (was 3 in parent L2_fuse_fwd_combined)
#   Forward: chunk_fwd_combined (1 call)
#   Backward: chunk_gla_bwd_fuse_dh_v2 (1 call, was 2 calls)
# ============================================================
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        # MUTATION (bwd_fuse_dh_v2): Single fused pallas_call replacing
        # chunk_bwd_dh_pallas + chunk_gla_bwd_fused (2 calls -> 1 call).
        # Fix: 5D BlockSpec (1,1,1,BK,BV) for 5D h array [B,H,NT,K,V].
        dq, dk, dv = chunk_gla_bwd_fuse_dh_v2(
