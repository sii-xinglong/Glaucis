"""GMM block-wise FP8 quantization kernel for TPU -- tiling_phase_specialization variant.

Builds on SO8 (zero backward quantization, 1.974x) with phase-specialized
tiling that matches each operation's matrix dimensions more precisely.
...
SO8 used identical (1024, 256, 128) for fwd and bwd_gmm, and (2048, 256, 128)
for tgmm. This variant explores different tiling per phase:
  - Forward gmm:  (256, 256, 128) -- TM=256 matches per-group M exactly,
    TK=256 for less register pressure (smaller K tiles)
  - Backward gmm: (1024, 256, 128) -- keep SO8's proven optimal tiling
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,256,128, 1024,256,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 256, 128, 1024, 256, 128, 2048, 256, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    tiling_phase_specialization variant: phase-specialized tiling with
    fwd (256,256,128), bwd_gmm (1024,256,128), tgmm (2048,512,128).
