...
def chunk_gla_fwd(q, k, v, g_gamma, scale, chunk_size):
    ...
    # g_cumsum still needed as a residual for backward (used in _bwd)
...
#
# MUTATION (algebraic_simplify): Eliminates g_cumsum input from the
# backward kernel. Instead, all gating is derived from g_gamma scalar.
#
# Key algebraic simplifications:
#   1. g_cumsum dropped from backward inputs/BlockSpec — compute
#      b_g_ramp = g_gamma[i_h] * (arange(BT)+1) as [BT] vector
#   2. exp values computed as [BT] vectors instead of [BT,K]:
#      exp_pos = exp(b_g_ramp) as [BT], broadcast at point of use
#   3. dq factored: exp_pos * (b_dq_intra_raw + scale * b_dq_inter)
#      eliminates one [BT,K] intermediate
#   4. dk factored: exp_neg * (b_dk_intra_raw + b_dk_inter * exp_gn)
#      reduces from 2 different [BT,K] exp arrays to 1 + scalar
#   5. Total: 3 exp() of [BT,K] -> 2 exp() of [BT] + arithmetic
#
# Architecture:
#   - Inputs q/k/v/do: 4D [B, H, NT*BT, dim], pre-flipped for reverse scan
#   - h input: 5D [B, H, NT, K, V], pre-flipped along axis=2
#   - g_gamma: scalar per head via SMEM (replaces g_cumsum 4D input)
#   - Output: 5D [B, NT, H, BT, K/V], block shape covers all NT
#   - Grid: (B, H, 1, 1, NT)
# ============================================================


def _chunk_gla_bwd_algebraic_simplify_kernel(
    q_ref, k_ref, v_ref, h_ref, do_ref, g_gamma,
    dq_ref, dk_ref, dv_ref, scratch_ref,
    *, BT, NT, scale,
):
    """Fused backward kernel with algebraic simplification.

    Key change: g_cumsum input eliminated. All gating derived from g_gamma
    scalar. Exp computations use [BT] vectors instead of [BT,K] arrays.
    dq and dk factored to reduce intermediate liveness.
    ...
    # -------------------------------------------------------
    # Gate computation from g_gamma scalar (replaces g_cumsum load)
    # b_g_ramp is [BT] instead of [BT, K] — all K columns identical
    # -------------------------------------------------------
    g_gamma_h = g_gamma[i_h].astype(jnp.float32)
    b_g_ramp = g_gamma_h * (jnp.arange(0, BT) + 1)     # [BT]
    b_g_last = g_gamma_h * BT                             # scalar: g_gamma * BT
    b_gn = b_g_ramp[BT - 1]                              # scalar: g_gamma * BT (same as b_g_last)
    ...
    # -------------------------------------------------------
    # Phase 0 (VPU): Compute exp values as [BT] vectors
    # This replaces three [BT,K] exp arrays with two [BT] vectors.
    # -------------------------------------------------------
    exp_pos = jnp.exp(b_g_ramp)                # [BT]  (was exp_pos_g [BT,K])
    exp_neg = jnp.exp(-b_g_ramp)               # [BT]  (was exp_neg_g [BT,K])
    exp_gn_scalar = jnp.exp(b_gn)             # scalar: exp(g_gamma * BT)

    # Gated q and k: broadcast [BT] -> [BT, K] at point of use
    q_pos = (b_q * exp_pos[:, None]).astype(b_q.dtype)           # [BT, K]: q * exp(g)
    k_neg = (b_k * exp_neg[:, None]).astype(b_k.dtype)           # [BT, K]: k * exp(-g)
    # k_decay = k * exp(gn - g) = k * exp(gn) * exp(-g) = k * exp_gn_scalar * exp_neg
    k_decay = (b_k * (exp_gn_scalar * exp_neg)[:, None]).astype(b_k.dtype)  # [BT, K]
    ...
    # -------------------------------------------------------
    # Phase 5 (VPU): Combine results with algebraic factoring
    #
    # FACTORED dq: Instead of two separate [BT,K] products:
    #   b_dq = b_dq_intra_raw * exp_pos_g + b_dq_inter * scale * exp_pos_g
    # Factor out exp_pos (broadcast [BT] -> [BT,K]):
    #   b_dq = exp_pos[:, None] * (b_dq_intra_raw + scale * b_dq_inter)
    # This eliminates one [BT,K] intermediate.
    #
    # FACTORED dk: Instead of two different [BT,K] exp arrays:
    #   b_dk = b_dk_intra_raw * exp_neg_g + b_dk_inter * exp_gn_minus_g
    # Factor out exp_neg (broadcast [BT] -> [BT,K]):
    #   b_dk = exp_neg[:, None] * (b_dk_intra_raw + b_dk_inter * exp_gn_scalar)
    # This reduces from 2 different exp arrays to 1 + scalar.
    # -------------------------------------------------------
    ...
    # q_hat = q * exp(b_g_ramp) * scale — reuse exp_pos [BT] vector
    # -------------------------------------------------------
    scratch_ref[...] *= jnp.exp(b_g_last)

    q_hat = (b_q * exp_pos[:, None] * scale).astype(jnp.float32)
...
def chunk_gla_bwd_algebraic_simplify(q, k, v, g_cumsum, h, do, g_gamma, scale, chunk_size):
    """Fused backward with algebraic simplification.

    MUTATION (algebraic_simplify): Eliminates g_cumsum from the Pallas kernel.
    All gating derived from g_gamma scalar. Exp computations use [BT] vectors
    instead of [BT,K] arrays. dq and dk factored to reduce intermediates.

    Signature unchanged (g_cumsum still accepted but NOT passed to kernel).

    Reverse scan: inputs q/k/v/do pre-flipped along chunk axis (axis=2 in
    ...
    # NOTE: g_cumsum NOT flattened/passed — eliminated from kernel inputs
    ...
    dq_5d, dk_5d, dv_5d = pl.pallas_call(
        functools.partial(
            _chunk_gla_bwd_algebraic_simplify_kernel,
...
# custom_vjp wrapper
#
# MUTATION (algebraic_simplify + fuse_fwd_combined):
#   _fwd: uses chunk_fwd_combined (single pallas_call for forward)
#   _bwd: uses chunk_gla_bwd_algebraic_simplify (single pallas_call
#         for backward — g_cumsum eliminated from kernel, gating from
#         g_gamma scalar, exp as [BT] vectors, factored dq/dk)
#
# Total pallas_calls: 2 (unchanged from parent)
# ============================================================
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        # MUTATION (algebraic_simplify): Single fused pallas_call.
        # g_cumsum passed for API compat but NOT used inside kernel.
        # All gating derived from g_gamma scalar directly.
        dq, dk, dv = chunk_gla_bwd_algebraic_simplify(
