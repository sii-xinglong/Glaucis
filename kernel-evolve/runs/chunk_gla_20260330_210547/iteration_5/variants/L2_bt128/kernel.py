...
# Entry point
#
# MUTATION (bt128): Override chunk_size to 128 regardless of
# evaluator config. This halves NT (64 -> 32), halving grid
# iterations for all time-sequential kernels (fwd_h, bwd_dh).
# Matmul tiles become [128,128] @ [128,128] -- better MXU
# utilization than [64,128] @ [128,64].
# ============================================================
...
def optimized_compute(B=2, T=4096, H=16, K=128, V=128, chunk_size=64):
    ...
    MUTATION (bt128): chunk_size overridden to 128. The evaluator
    passes chunk_size=64 from config, but we use 128 to halve the
    number of time chunks (NT: 64 -> 32), reducing grid iterations
    and kernel launch overhead.
    """
    chunk_size = 128  # Override: BT=128 for fewer grid iterations and larger MXU matmuls
