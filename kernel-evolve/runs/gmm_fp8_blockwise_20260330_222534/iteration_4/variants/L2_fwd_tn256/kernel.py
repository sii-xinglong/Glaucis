"""GMM block-wise FP8 quantization kernel for TPU -- fwd_tn256 variant.

Builds on L2_bwd_tm256_tk512 which combined TM=256 with TK=512 for bwd_gmm.
This variant increases forward TN from 128 to 256, halving the N-grid
iterations in the forward pass.
...
For Gate/Up (N=512), TN=256 halves N-grid from 4 to 2 tiles.
For Down (N=2048), TN=256 reduces N-grid from 16 to 8 tiles.
Forward M*N tile product: 256*256=64K, well within ~256K limit.

Phase-specialized tiling: (256,256,256, 256,512,128, 2048,512,128)
  - Forward gmm:  (256, 256, 256) -- TM=256 matches per-group M exactly,
    TN=256 halves N-grid iterations, TK=256 for register pressure control
  - Backward gmm: (256, 512, 128) -- TM=256 for per-group alignment +
    TK=512 halves K-loop iterations (K=2048: 4 iters instead of 8)
  - tgmm:         (2048, 512, 128) -- TK=512 (bf16, no fp8 constraints)
    to reduce K-loop iterations for tgmm
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 256, 256, 256, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    fwd_tn256 variant: phase-specialized tiling with
    fwd (256,256,256), bwd_gmm (256,512,128), tgmm (2048,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Forward gmm with phase-specialized tiling: (256, 256, 256)
    # TM=256 matches per-group M exactly (M=8192 / G=32 = 256)
    # TN=256 halves N-grid iterations vs TN=128
