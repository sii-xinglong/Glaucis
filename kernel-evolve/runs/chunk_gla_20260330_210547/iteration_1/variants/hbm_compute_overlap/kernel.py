...
# Optimization: pltpu.emit_pipeline to overlap HBM DMA with MXU compute
# ============================================================


def _chunk_fwd_h_kernel_body(
    k_ref, v_ref, h0_ref, g_gamma,
    h_ref, ht_ref, scratch_ref,
    *, BT, NT,
):
    """Inner kernel body used inside emit_pipeline.

    When called via emit_pipeline, k_ref and v_ref are already in VMEM
    (the pipeline manages double-buffering transparently), so MXU compute
    overlaps with the next tile's DMA transfer.
    """
    BK = k_ref.shape[3]
    BV = v_ref.shape[3]
    i_b, i_h, i_k, i_v, i_t = (
...
def _chunk_fwd_h_kernel(
    ...
    *, BT, NT,
    ...
    """Inter-chunk state propagation kernel with DMA/compute overlap via emit_pipeline.

    emit_pipeline wraps the sequential i_t loop (the innermost grid dimension)
    and schedules DMA prefetch of the next k/v tile while the MXU processes
    the current tile. The compiler manages the double-buffer slots automatically,
    avoiding manual scratch allocation that would worsen register pressure.
    """
    BK = k_ref.shape[3]
    BV = v_ref.shape[3]
    i_b, i_h, i_k, i_v = (
    ...
    # Initialize accumulator state before the pipeline loop
    @pl.when(True)
    def do_init():
        if h0_ref is not None:
            scratch_ref[:, :] = h0_ref[0, 0].astype(jnp.float32)
    ...
    def pipeline_body(i_t, k_tile_ref, v_tile_ref):
        """Body executed for each time step inside the pipeline."""
        # Save current state before update (needed by fwd pass output)
        h_ref[0, i_t, 0] = scratch_ref[...]

        k_tile = k_tile_ref[0, 0]
        v_tile = v_tile_ref[0, 0]

        b_g_last = g_gamma[i_h] * BT
        scratch_ref[...] *= exp(b_g_last)
        v_decay = (v_tile * exp(b_g_last - b_g)[:, None]).astype(v_tile.dtype)
    ...
    # Use emit_pipeline to wrap the time-step loop.
    # The pipeline prefetches k_ref[..., i_t+1, ...] and v_ref[..., i_t+1, ...]
    # via DMA while the MXU/VPU compute on tile i_t is in flight.
    # This targets the 24-DMA-no-double-buffering bottleneck in the profile.
    pltpu.emit_pipeline(
        pipeline_body,
        in_specs=[
            pl.BlockSpec((1, 1, BT, BK), lambda i_t: (i_b, i_h, i_t, i_k)),
            pl.BlockSpec((1, 1, BT, BV), lambda i_t: (i_b, i_h, i_t, i_v)),
        ],
        out_specs=[],
        grid=(NT,),
    )(k_ref, v_ref)
...
def chunk_fwd_h(k, v, g_gamma, chunk_size):
    ...
    # Grid: (B, H, K-blocks, V-blocks) — the i_t loop is handled inside
    # emit_pipeline, so NT is no longer a grid dimension. This collapses
    # the outer grid and lets the pipeline manage the sequential time dimension.
    grid = (B, H, pl.cdiv(K_dim, BK), pl.cdiv(V, BV))

    def k_map(b, h, ki, vi): return b, h, 0, ki
    def v_map(b, h, ki, vi): return b, h, 0, vi
    def h_map(b, h, ki, vi): return b, 0, h, ki, vi
    def ht_map(b, h, ki, vi): return b, h, ki, vi
    ...
    h_all, ht = pl.pallas_call(
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            grid=grid,
            in_specs=[
                # Full k and v tensors provided to the kernel; emit_pipeline
                # internally slices along the time dimension with DMA prefetch.
                pl.BlockSpec((1, H, NT * BT, BK), k_map),
                pl.BlockSpec((1, H, NT * BT, BV), v_map),
                None,
