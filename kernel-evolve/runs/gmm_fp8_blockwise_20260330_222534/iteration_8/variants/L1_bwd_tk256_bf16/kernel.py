"""GMM block-wise FP8 quantization kernel for TPU -- L1_bwd_tk256_bf16 variant.

Testing bwd TK=256 with bf16 scales. bf16 scales reduce register pressure
(SO15), which might change the optimal bwd TK. TK=512 was optimal with f32
scales but bf16 scales may allow simpler bwd loops to compile more efficiently.

Builds on L1_scale_bf16 which proved that scale_dtype=bfloat16 reduces scale
tensor size by 2x and leverages native bf16 support on TPU v7x.

This variant changes backward gmm TK from 512 to 256, testing whether the
reduced register pressure from bf16 scales shifts the optimal backward tiling.

Tiling vs parent L1_scale_bf16:
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 256, 128) -- TK 512->256
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,256,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 256, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_bwd_tk256_bf16 variant: bwd TK 512->256 with scale_dtype bfloat16.
    fwd (256,512,256), bwd_gmm (256,256,128), tgmm (2048,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (256, 256, 128) -- TK=256 (reduced from 512)
    # bf16 scales reduce register pressure, testing if smaller TK compiles
    # more efficiently with the reduced pressure.
