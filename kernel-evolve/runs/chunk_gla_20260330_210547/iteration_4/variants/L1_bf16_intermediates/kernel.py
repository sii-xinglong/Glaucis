...
MUTATION (bf16_intermediates): Cast intermediate [BT,BT] matrices (b_a_masked,
b_dA) to bfloat16 before feeding them into matmuls. On TPU, bfloat16 uses half
the register space of float32. Matmuls with precision=HIGHEST accumulate in
float32 internally regardless of input dtype, so casting inputs to bf16 loses
minimal precision while halving register pressure for those arrays. Also ensures
k_neg, k_decay, q_pos stay in bfloat16 immediately after exp multiplication.
"""
...
#
# MUTATION (bf16_intermediates): b_A_masked is already cast to b_v.dtype
# (bfloat16) before the intra-chunk matmul — this is correct and maintained.
#
...
def _chunk_gla_fwd_o_gk_pl(q_ref, k_ref, v_ref, g_ref, h_ref, o_ref, *, BT, scale):
    ...
    # MUTATION (bf16_intermediates): Cast masked A to bfloat16 before matmul.
    # The subsequent matmul accumulates in float32 via precision=HIGHEST,
    # so bf16 inputs lose minimal precision while halving register footprint
    # for this [BT, BT] = [64, 64] matrix (16KB -> 8KB).
...
#
# MUTATION (combined: reduce_inputs + skip_dg + bf16_intermediates):
#   1. A is recomputed inside the kernel (from L1_reduce_inputs parent)
#   2. dg computation is completely removed (skip_dg)
#   3. Cast [BT,BT] intermediates (b_a_masked, b_dA) to bfloat16
#      before matmul inputs to halve register pressure (bf16_intermediates)
#
# bf16_intermediates rationale:
#   - b_a_masked [64,64] float32 = 16KB -> bfloat16 = 8KB
#   - b_dA [64,64] float32 = 16KB -> bfloat16 = 8KB
#   - Total saving: 16KB of register space per kernel invocation
#   - Matmuls with precision=HIGHEST accumulate in float32 internally
#     regardless of input dtype, so bf16 inputs lose minimal precision
#   - k_neg, k_decay, q_pos already cast to input dtype (bfloat16)
#     immediately after exp multiplication — verified and maintained
#
# MXU-VPU overlap strategy:
...
#   Phase 2 (VPU): Apply causal masks to both A and dA, cast to bf16
#   Phase 3 (MXU batch): Four independent dot products back-to-back
#   Phase 4 (MXU): Intra-chunk dq and dk (with bf16 b_dA, k_neg inputs)
...
def _chunk_gla_bwd_fused_kernel(
    ...
    *, BT, scale,
    ...
    # Pre-cast gated key/query variants to bfloat16 immediately
    # after exp multiplication. These are [BT, K] = [64, 128] arrays.
    # Keeping them in bfloat16 halves their register footprint (32KB -> 16KB each).
    ...
    b_dA_raw = jnp.dot(b_do.astype(jnp.bfloat16), b_v.T,
                       precision=jax.lax.Precision.HIGHEST,
                       preferred_element_type=jnp.float32) * scale  # [BT, BT]

    # -------------------------------------------------------
    # Phase 2 (VPU): Apply causal masks, then cast to bfloat16
    # MUTATION (bf16_intermediates): After masking, cast b_a_masked
    # and b_dA to bfloat16. These [64,64] matrices go from 16KB to
    # 8KB each in registers. The subsequent matmuls accumulate in
    # float32 via precision=HIGHEST regardless of input dtype.
    # -------------------------------------------------------
    mask = jnp.arange(BT)[:, None] >= jnp.arange(BT)[None, :]
    b_dA = jnp.where(mask, b_dA_raw, 0.0).astype(jnp.bfloat16)      # [BT, BT] bf16
    b_a_masked = jnp.where(mask, b_a, 0.0).astype(jnp.bfloat16)     # [BT, BT] bf16

    # -------------------------------------------------------
    # Phase 3 (MXU batch): Four independent dot products
    # b_a_masked and b_dA are now bfloat16, saving register space.
    # k_neg, k_decay, q_pos are already bfloat16.
    # -------------------------------------------------------
    b_dv_intra = jnp.dot(b_a_masked.T, b_do.astype(jnp.bfloat16),
                         precision=jax.lax.Precision.HIGHEST,
    ...
    b_dq_inter = jnp.dot(b_do.astype(jnp.bfloat16), b_h.astype(jnp.bfloat16).T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)   # [BT, K]

    b_dk_inter = jnp.dot(b_v, b_dh.astype(jnp.bfloat16).T,
                         precision=jax.lax.Precision.HIGHEST,
                         preferred_element_type=jnp.float32)   # [BT, K]

    # -------------------------------------------------------
    # Phase 4 (MXU): Intra-chunk dq and dk
    # b_dA and k_neg/q_pos are all bfloat16 — register pressure
    # is reduced vs float32 originals.
    # -------------------------------------------------------
    ...
    # exp_neg_g and exp_gn_minus_g are float32, results stay float32
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    ...
    MUTATION (combined):
      ...
      - bf16_intermediates: b_a_masked, b_dA cast to bf16 before matmuls
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
