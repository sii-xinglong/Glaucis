...
# Variant: L1_scratch_A — VMEM scratch buffer for [BT,BT] attention matrix in forward kernel
"""fused_chunk_simple_gla Pallas TPU kernel — L1_scratch_A variant.
...
Forward kernel: 4-step grid unrolling with VMEM scratch for attention matrix A.
Grid is (B, H, NK, NV, NT//4) instead of (B, H, NK, NV, NT).

Key optimization: The [BT,BT] = [64,64] attention matrix `partial_A` (16KB at f32)
is a significant register consumer across all 4 sub-steps. With VMEM at only 0.88%
utilization (576 KB of 64 MiB), there is massive headroom. By adding a second VMEM
scratch buffer for A and explicitly storing/loading it from VMEM between computation
and use, we force the compiler to stage A in VMEM instead of registers. This should
reduce the 6.3M register spills by freeing registers that were holding the [BT,BT]
attention matrix.
...
# VMEM scratch for [BT,BT] attention matrix to reduce register spills
# =============================================================================
...
def _fused_chunk_fwd_4step_kernel(
    ...
    scratch_A_ref,  # (BT, BT) VMEM float32 — attention matrix staging buffer
    *,
    BT: int,
    NT: int,
    NT_QUARTER: int,
    ...
    # Stage A through VMEM scratch to free registers
    scratch_A_ref[:, :] = partial_A0
    partial_A0_staged = scratch_A_ref[:, :]

    partial0 = partial_o0 + jnp.dot(
        partial_A0_staged, b_v0.astype(jnp.float32),
    ...
    # Stage A through VMEM scratch to free registers
    scratch_A_ref[:, :] = partial_A1
    partial_A1_staged = scratch_A_ref[:, :]

    partial1 = partial_o1 + jnp.dot(
        partial_A1_staged, b_v1.astype(jnp.float32),
        precision=jax.lax.Precision.HIGHEST,
        preferred_element_type=jnp.float32,
    )
    ...
    # Stage A through VMEM scratch to free registers
    scratch_A_ref[:, :] = partial_A2
    partial_A2_staged = scratch_A_ref[:, :]

    partial2 = partial_o2 + jnp.dot(
        partial_A2_staged, b_v2.astype(jnp.float32),
        precision=jax.lax.Precision.HIGHEST,
        preferred_element_type=jnp.float32,
    )
    ...
    # Stage A through VMEM scratch to free registers
    scratch_A_ref[:, :] = partial_A3
    partial_A3_staged = scratch_A_ref[:, :]

    partial3 = partial_o3 + jnp.dot(
        partial_A3_staged, b_v3.astype(jnp.float32),
        precision=jax.lax.Precision.HIGHEST,
        preferred_element_type=jnp.float32,
    )
    ...
    o_partial, ht = pl.pallas_call(
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            scratch_shapes=[
                ...
                pltpu.VMEM((BT, BT), jnp.float32),    # scratch_A_ref: attention matrix staging
            ],
        ),
