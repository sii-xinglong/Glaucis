...
# Variant: L1_k_split_h — bf16 dh_states storage to halve HBM traffic in backward
"""fused_chunk_simple_gla Pallas TPU kernel — L1_k_split_h variant.
...
Forward kernel: 4-step grid unrolling WITHOUT scratch_A (cleaned up from
L1_scratch_A). The [BT,BT] attention matrix staging through VMEM scratch was
shown to have no measurable benefit over compiler-managed register allocation.

Backward optimization: dh_states intermediate tensor stored as bf16 instead of
f32. The dh_states tensor is [B,H,NT,K,V] = [B,H,NT,128,128]. At f32 this is
a large HBM intermediate (~6.4GB for B=12,H=16,NT=64). Storing as bf16 halves
HBM write bandwidth in Pass 1 and read bandwidth in Pass 2.

Key correctness argument: dh_states are SNAPSHOTS of the dh accumulator at each
time step, written once by Pass 1 and consumed once by Pass 2. They are NOT
further accumulated, so bf16 precision loss is bounded to a single
truncation (analogous to storing h residuals in bf16, per SO17 pattern).
Pass 2 immediately casts back to f32 on load before any computation.

Changes from L1_scratch_A:
1. Removed scratch_A_ref from forward kernel (cleanup)
2. Pass 1 (_bwd_dv_dh_kernel): cast dh to bf16 before storing to dh_states
3. Pass 1 out_shapes: dh_states dtype changed from f32 to bf16
4. Pass 2 (_bwd_dq_dk_kernel): explicit .astype(jnp.float32) on dh load (was
   already present, now converts from bf16)
"""
...
# scratch_A removed: compiler manages [BT,BT] attention matrix in registers
# =============================================================================
...
def _bwd_dv_dh_kernel(
    ...
    *, BT: int, NT: int, scale: float,
    ...
    dh_states are stored as bf16 to halve HBM write bandwidth. The dh snapshots
    are not further accumulated, so a single bf16 truncation is acceptable.
    """
    K = q_ref.shape[3]
    V = v_ref.shape[3]
    i_t = pl.program_id(2)
    ...
    # Cast to bf16 to halve HBM write bandwidth for this large intermediate
    dh_states_ref[0, 0, 0] = scratch_ref[...].astype(jnp.bfloat16)
...
def _bwd_dq_dk_kernel(
    ...
    *, BT: int, NT: int, scale: float,
    ...
    dh_states are loaded from bf16 and cast to f32 before use.
    """
    K = q_ref.shape[3]
    V = v_ref.shape[3]
    i_t = pl.program_id(2)
    ...
    b_do = do_ref[0, 0]
    # Load bf16 dh_states and upcast to f32 for computation
...
def _fused_chunk_bwd_launcher(
    ...
    # dh_states stored as bf16 to halve HBM traffic.
    # =========================================================================
    pass1_in_specs = [
    ...
    pass1_out_shapes = [
        ...
        jax.ShapeDtypeStruct((B, H, NT, K, V), jnp.bfloat16),   # dh_states as bf16
    ]
    ...
    # dh_states loaded from bf16, upcast to f32 inside kernel.
    # =========================================================================
    pass2_in_specs = [
        pl.BlockSpec((1, 1, BT, K), rev_qk_map),    # q
        pl.BlockSpec((1, 1, BT, K), rev_qk_map),    # k
        ...
        pl.BlockSpec((1, 1, BT, V), rev_v_map),      # do
        pl.BlockSpec(memory_space=smem),              # g_gamma
        pl.BlockSpec((1, 1, 1, K, V), rev_h_map),    # dh_states from Pass 1 (bf16)
    ]
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    Pass 1: dv + dh propagation — Phase A inter-chunk (k_decay@dh), Phase B intra-chunk (A^T@do)
      dh_states stored as bf16 to halve HBM write bandwidth.
    Pass 2: dq + dk — Phase A inter-chunk (do@h^T, v@dh^T), Phase B intra-chunk (dA_gated@k/q)
      dh_states loaded from bf16, upcast to f32 for computation.
    """
    @jax.custom_vjp
    def _compute(q, k, v):
