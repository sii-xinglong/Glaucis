"""GMM block-wise FP8 quantization kernel for TPU -- L2_fwd_tn512 variant.
...
This variant increases forward TN from 256 to 512, doubling the N-tile size
in the forward gmm pass. For Gate/Up (N=512), this means a single N-tile
covering the full N dimension. For Down (N=2048), 4 iterations instead of 8.

Tiling changes vs parent:
  - Forward gmm:  (256, 512, 512) -- TN 256->512
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,512, 256,512,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 512, 256, 512, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_fwd_tn512 variant: forward TN 256->512 to further reduce N-loop iterations.
    fwd (256,512,512), bwd_gmm (256,512,128), tgmm (2048,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Forward gmm with phase-specialized tiling: (256, 512, 512)
    # TM=256 matches per-group M exactly (M=8192 / G=32 = 256)
    # TK=512 halves K-loop iterations (K=2048: 4 iters instead of 8)
    # TN=512 covers full N for Gate/Up (N=512: 1 iter), Down (N=2048: 4 iters)
