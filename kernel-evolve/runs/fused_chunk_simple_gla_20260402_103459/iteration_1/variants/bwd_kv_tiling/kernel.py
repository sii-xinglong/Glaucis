...
# Variant: bwd_kv_tiling — two-phase backward with separated inter/intra-chunk computation
"""fused_chunk_simple_gla Pallas TPU kernel — bwd_kv_tiling variant.
...
Forward kernel: 4-step grid unrolling (UNCHANGED from baseline).
Grid is (B, H, NK, NV, NT//4) instead of (B, H, NK, NV, NT).

Backward restructured with two-phase kernel bodies:
- Pass 1 (dv+dh): Phase A = inter-chunk dv via k_decay @ dh, Phase B = intra-chunk
  dv via A^T @ do. Separating the phases means dh and the attention matrix A are not
  simultaneously live, reducing peak register pressure.
- Pass 2 (dq+dk): Phase A = inter-chunk dq/dk using h and dh, Phase B = intra-chunk
  dq/dk using dA_gated. The inter-chunk h-dependent terms are computed first and
  written to registers while A is not yet constructed, then dA_gated is built for
  the intra-chunk terms. This avoids having h, dh, A, and dA_gated all live at once.

Profile motivation: backward has 310K register spills. The key insight is that in the
original kernels, all intermediates (h, dh, A [BT,BT], dA [BT,BT], k_decay, q_hat)
are simultaneously live. By separating into Phase A (inter) then Phase B (intra), we
ensure that when computing inter-chunk contributions (which need h, dh of size [K,V]),
the [BT,BT] attention matrices are not yet allocated. Conversely when computing
intra-chunk, the [K,V] inter-chunk temporaries have been reduced to [BT,K] outputs.
"""
