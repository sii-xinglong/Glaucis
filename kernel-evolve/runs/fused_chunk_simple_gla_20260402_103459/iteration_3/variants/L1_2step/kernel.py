...
# Variant: L1_2step — 2-step forward grid unrolling for register pressure reduction
"""fused_chunk_simple_gla Pallas TPU kernel — L1_2step variant.
...
Forward kernel: 2-step grid unrolling to reduce register pressure.
Grid is (B, H, NK, NV, NT//2) instead of (B, H, NK, NV, NT).

Key optimization: Profile shows 6.3M register spills dominating execution with
4-step unrolling. By reducing from 4 sub-steps to 2, each kernel body has fewer
simultaneously-live q/k/v/h/A/o intermediates. This trades more grid iterations
(NT//2 instead of NT//4) for simpler per-iteration code with roughly half the
live values per invocation. No scratch_A_ref needed — fewer live values means
the attention matrix can stay in registers without excessive spills.

Backward restructured with two-phase kernel bodies (UNCHANGED from bwd_kv_tiling):
- Pass 1 (dv+dh): Phase A = inter-chunk dv via k_decay @ dh, Phase B = intra-chunk
  dv via A^T @ do.
- Pass 2 (dq+dk): Phase A = inter-chunk dq/dk using h and dh, Phase B = intra-chunk
  dq/dk using dA_gated.
"""
...
# =============================================================================
# Forward: Fused chunk forward kernel with 2-step time unrolling
# Grid: (B, H, NK, NV, NT//2) — each iteration processes 2 consecutive chunks
# Reduced from 4-step to halve live values per kernel invocation
# =============================================================================


def _fused_chunk_fwd_2step_kernel(
    q_ref,          # (1, 1, 2*BT, BK)  — K-tiled, 2 chunks
    k_ref,          # (1, 1, 2*BT, BK)  — K-tiled, 2 chunks
    v_ref,          # (1, 1, 2*BT, BV)  — V-tiled, 2 chunks
    h0_ref,         # (1, 1, BK, BV)  or None
    g_ref,          # (1, 1, 2*BT, 128) or None
    g_gamma_ref,    # [H] via SMEM/ANY, or None
    o_ref,          # (1, 1, 1, 2*BT, BV)  — partial output, 2 chunks
    ht_ref,         # (1, 1, BK, BV)   — output, or None
    scratch_ref,    # (BK, BV) VMEM float32 — h state accumulator
    *,
    BT: int,
    NT: int,
    NT_HALF: int,
    ...
    # --- 2-step unrolled loop: process 2 consecutive chunks ---
    for step in range(2):
        t_offset = step * BT

        b_q = q_ref[0, 0, pl.ds(t_offset, BT), :]
        b_k = k_ref[0, 0, pl.ds(t_offset, BT), :]
        b_v = v_ref[0, 0, pl.ds(t_offset, BT), :]
        b_h = scratch_ref[...]
        ...
        if g_ref is not None:
            b_g = g_ref[0, 0, pl.ds(t_offset, BT), 0].astype(jnp.float32)
        ...
        o_ref[0, 0, 0, pl.ds(t_offset, BT), :] = partial.astype(o_ref.dtype)
    ...
    # Store final h state at the last 2-step iteration
...
def fused_chunk_fwd_kernel(
    ...
    *,
    ...
    assert NT % 2 == 0, f"NT={NT} must be divisible by 2 for 2-step unrolling"
    NT_HALF = NT // 2
    ...
    # 2-step unrolled grid: NT_HALF iterations instead of NT
    grid = (B, H, NK, NV, NT_HALF)

    # BlockSpecs now tile 2*BT in the time dimension
    ...
    # Time dimension tiles are 2*BT to cover 2 chunks per iteration
    spec_qk    = pl.BlockSpec((1, 1, 2 * BT, BK), qk_map)
    spec_v     = pl.BlockSpec((1, 1, 2 * BT, BV), v_map)
    spec_h0    = pl.BlockSpec((1, 1, BK, BV), state_map) if h0 is not None else None
    spec_g     = pl.BlockSpec((1, 1, 2 * BT, 128), g_map) if _g is not None else None
    spec_gamma = pl.BlockSpec(memory_space=smem) if g_gamma is not None else None

    # Output covers 2*BT per iteration
    spec_o     = pl.BlockSpec((1, 1, 1, 2 * BT, BV), o_map)
    spec_ht    = pl.BlockSpec((1, 1, BK, BV), state_map) if output_final_state else None

    # out_shape for o_partial: time dim stays T (NT_HALF * 2*BT = NT * BT = T)
    out_shapes = [
    ...
    o_partial, ht = pl.pallas_call(
        functools.partial(
            _fused_chunk_fwd_2step_kernel,
            BT=BT, NT=NT, NT_HALF=NT_HALF,
        ),
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            scratch_shapes=[
                pltpu.VMEM((BK, BV), jnp.float32),    # scratch_ref: h state accumulator
            ],
        ),
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    Forward uses fused kernel with 2-step time unrolling (h stays in VMEM,
    single pallas_call, NT//2 grid iterations instead of NT).
