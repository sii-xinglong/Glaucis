"""GMM block-wise FP8 quantization kernel for TPU -- L1_scale_bf16 variant.

Builds on L1_fwd_tn256 (2.751x) which proved forward TN=256 effective with
phase-specialized tiling (256,512,256, 256,512,128, 2048,512,128).

This variant changes the scale_dtype for BOTH quantize() calls in _gmm_fwd
from jnp.float32 to jnp.bfloat16. This reduces scale tensor size by 2x,
potentially improving memory bandwidth and reducing VPU overhead for scale
manipulation. bfloat16 scales have sufficient dynamic range for absmax
calibration while being natively supported by TPU v7x.

Tiling unchanged vs parent:
...
def gmm_fp8_blockwise(
    ...
    L1_scale_bf16 variant: scale_dtype float32->bfloat16 for both quantizations.
