...
# Variant: L1_bt32 — BT=32 chunk size with 4-step forward grid unrolling
"""fused_chunk_simple_gla Pallas TPU kernel — L1_bt32 variant.
...
Forward kernel: 4-step grid unrolling with BT=32 (chunk_size=32).
Grid is (B, H, NK, NV, NT//4) instead of (B, H, NK, NV, NT).
With T=4096 and BT=32: NT=128, NT//4=32 iterations.

Key optimization: BT=32 creates smaller [32,32] attention matrices (4KB at f32)
compared to [64,64] (16KB at f32), significantly reducing per-sub-step live
values and register pressure. Each 4-step iteration processes 4x32=128 time
steps. The tradeoff is potential MXU underutilization since [32,128] matmul
tiles use only 1/4 of 128-wide MXU rows.

No scratch_A_ref buffer (removed per FP33: VMEM scratch staging for A was
counterproductive).
...
# BT=32: smaller attention matrices [32,32] to reduce register pressure
# =============================================================================
...
def fused_chunk_fwd(
    ...
    use_ht=False, chunk_size=32, interpret=None,
):
    B, T, H, K = q.shape
    V = v.shape[-1]
...
def fused_chunk_simple_gla_fwd(
    ...
    scale=None, use_ht=False, chunk_size=32, interpret=None,
):
...
def chunk_fwd_h(
    ...
    chunk_size=32, split_size=None, states_in_fp32=False,
):
    BK = 128
    BV = 128
...
def optimized_compute(B=2, T=4096, H=16, K=128, V=128, chunk_size=64):
    ...
    BT=32 variant: smaller [32,32] attention matrices to reduce register pressure.
    Returns the loss scalar. Both forward and backward are computed,
    so timing captures the full training step performance.
    """
    chunk_size = 32  # Force BT=32 regardless of input
