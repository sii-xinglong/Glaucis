"""GMM block-wise FP8 quantization kernel for TPU -- L2_tgmm_tm128 variant.

Builds on L2_fwd_tn256_tk512 (2.769x) which proved forward TN=256 effective with
phase-specialized tiling (256,512,256, 256,512,128, 2048,512,128).

L2_tgmm_tm128 -- testing smallest possible tgmm TM (128). Creates 64 tgmm tiles
per group. Tests if minimum-size tgmm tiles have less overhead than larger tiles.

Tiling changes vs parent:
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (128, 512, 128) -- TM 1024->128
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,128, 128,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 512, 128, 128, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_tgmm_tm128 variant: tgmm TM 1024->128 to test minimum-size tgmm tiles.
    fwd (256,512,256), bwd_gmm (256,512,128), tgmm (128,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (128, 512, 128) -- TM 1024->128 to test minimum-size tiles.
    # With M=8192 and G=32, per-group M=256, so TM=128 creates 2 tiles per group
    # (64 total across all groups). Tests if smallest tiles reduce overhead.
