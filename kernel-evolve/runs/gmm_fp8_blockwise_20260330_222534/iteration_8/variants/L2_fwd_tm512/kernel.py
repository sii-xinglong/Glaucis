"""GMM block-wise FP8 quantization kernel for TPU -- L2_fwd_tm512 variant.

L2_fwd_tm512 — testing fwd TM=512 (2× per-group M) on the simplified tgmm
kernel. Previous TM=256 was optimal (SO9) but that was before tgmm TM=1024
simplification. Tests if the compiler schedules differently with less tgmm
VLIW pressure.

Tiling changes vs parent (L2_tgmm_tm1024):
  - Forward gmm:  (512, 512, 256) -- TM 256->512
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (512,512,256, 256,512,128, 1024,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (512, 512, 256, 256, 512, 128, 1024, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_fwd_tm512 variant: fwd TM 256->512 to test 2× per-group M tiling.
    fwd (512,512,256), bwd_gmm (256,512,128), tgmm (1024,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Forward gmm with phase-specialized tiling: (512, 512, 256)
    # TM=512 is 2× per-group M (M=8192 / G=32 = 256), testing if larger
    # M-tile improves compiler scheduling with reduced tgmm VLIW pressure
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (1024, 512, 128) -- TM=1024, unchanged from parent.
