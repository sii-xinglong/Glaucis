"""GMM block-wise FP8 quantization kernel for TPU -- L1_bwd_tn256 variant.
...
This variant increases backward TN from 128 to 256, doubling the N-tile size
in the backward gmm pass. Since bwd_gmm uses transpose_rhs=True, the effective
output dimension changes. Larger TN may improve memory access patterns.

Tiling changes vs parent:
  - Forward gmm:  (256, 512, 128) -- unchanged
  - Backward gmm: (256, 512, 256) -- TN 128->256
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,128, 256,512,256, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 256, 512, 256, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_bwd_tn256 variant: backward TN 128->256 to halve N-loop iterations in bwd.
    fwd (256,512,128), bwd_gmm (256,512,256), tgmm (2048,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (256, 512, 256) -- TN increased from 128 to 256
