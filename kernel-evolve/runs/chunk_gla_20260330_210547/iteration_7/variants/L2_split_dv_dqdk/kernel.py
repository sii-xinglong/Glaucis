...
# ============================================================
# Backward: Split fused kernel into dv-only and dq/dk-only
#
# MUTATION (split_dv_dqdk): Replace the single bwd_fused kernel with
# two separate pallas_calls to reduce register pressure per kernel.
#
# Motivation: The fused backward kernel has ~2.5M register spills
# because ~24 arrays are simultaneously live. By splitting into:
#   - Kernel A: dv-only (4 fewer matmuls, simpler live-variable set)
#   - Kernel B: dq/dk-only (no dv logic, separate live-variable set)
# each kernel has fewer simultaneous live intermediates, potentially
# reducing spills from 2.5M to ~1M per kernel.
#
# Tradeoff: A is recomputed in both kernels (duplication), but this
# avoids storing A in HBM between kernels. At the current performance
# level (7.845x baseline), adding one pallas_call launch overhead
# (~2-3ms) may be more than offset by VPU throughput gains from
# reduced register spills.
# ============================================================


def _chunk_gla_bwd_dv_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, do_ref, dh_ref,
    dv_ref,
    *, BT, scale,
):
    """Backward dv-only kernel.

    Computes:
      1. Recompute A = (q*exp(g)) @ (k*exp(-g)).T * scale, causal mask it
      2. dv_intra = A_masked.T @ do
      3. dv_inter = k_decay @ dh  where k_decay = k * exp(gn - g)
      4. dv = dv_intra + dv_inter

    Does NOT compute dq or dk — fewer live intermediates.
    """
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    ...
    # -------------------------------------------------------
    # Phase 0 (VPU): Pre-compute exp/gate values for dv path
    # Only need exp(-g) and exp(gn-g); skip exp(g) (not needed for dv)
    # -------------------------------------------------------
    exp_neg_g = jnp.exp(-b_g)                              # [BT, K]
    exp_gn_minus_g = jnp.exp(b_gn[None, :] - b_g)         # [BT, K]
    exp_pos_g = jnp.exp(b_g)                               # [BT, K] — needed for q_pos in A

    # Gated variants for A recomputation
    k_neg = (b_k * exp_neg_g).astype(b_k.dtype)            # [BT, K]: k * exp(-g)
    q_pos = (b_q * exp_pos_g).astype(b_q.dtype)            # [BT, K]: q * exp(g)

    # k_decay for inter-chunk dv
    ...
    # -------------------------------------------------------
    # Phase 4 (VPU): Combine and write dv
    # -------------------------------------------------------
    dv_ref[0, 0] = (b_dv_intra + b_dv_inter).astype(dv_ref.dtype)


def chunk_gla_bwd_dv(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    """Launch dv-only backward Pallas kernel.

    MUTATION (split_dv_dqdk): Computes only dv. Kernel A of the split.
    Inputs mirror the original fused kernel; output is dv only.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    dv, = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dv_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
    ...
    def _unreshape_v(x):
        x = x.reshape(H, B, NT, BT, V)
        x = x.transpose(1, 0, 2, 3, 4)
        x = x.reshape(B, H, T, V)
        return x.transpose(0, 2, 1, 3)

    return _unreshape_v(dv)


def _chunk_gla_bwd_dqdk_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, do_ref, dh_ref,
    dq_ref, dk_ref,
    *, BT, scale,
):
    """Backward dq/dk-only kernel.

    Computes:
      1. Recompute A = (q*exp(g)) @ (k*exp(-g)).T * scale, causal mask it
      2. dA_raw = (do @ v.T) * scale, causal mask it
      3. dq_intra = dA @ k_neg, dq_inter = do @ h.T
      4. dk_intra = dA.T @ q_pos, dk_inter = v @ dh.T
      5. Combine: dq = dq_intra*exp(g) + dq_inter*scale*exp(g)
                  dk = dk_intra*exp(-g) + dk_inter*exp(gn-g)

    Does NOT compute dv — fewer live intermediates than the fused kernel.
    """
    b_q = q_ref[0, 0]
    b_k = k_ref[0, 0]
    b_v = v_ref[0, 0]
    ...
    # -------------------------------------------------------
    # Phase 3 (MXU batch): Inter-chunk dq and dk
    # These can overlap with Phase 4 since they are independent
    # -------------------------------------------------------
...
def chunk_gla_bwd_dqdk(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    """Launch dq/dk-only backward Pallas kernel.

    MUTATION (split_dv_dqdk): Computes only dq and dk. Kernel B of the split.
    Inputs mirror the original fused kernel; outputs are dq, dk only.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    dq, dk = pl.pallas_call(
        functools.partial(_chunk_gla_bwd_dqdk_kernel, BT=BT, scale=scale),
        grid=grid,
        out_shape=[
    ...
    return _unreshape_k(dq), _unreshape_k(dk)


# ============================================================
# custom_vjp wrapper
#
# MUTATION (split_dv_dqdk + fold_dh_pallas + fuse_fwd_combined):
#   _fwd: residuals no longer include A (fuse_fwd_A)
#   _bwd: uses chunk_bwd_dh_pallas instead of _chunk_bwd_dh_scan,
#          then calls chunk_gla_bwd_dv and chunk_gla_bwd_dqdk
#          separately instead of the single chunk_gla_bwd_fused
# ============================================================
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        dh = chunk_bwd_dh_pallas(q, do, g_gamma, scale, chunk_size)
        # MUTATION (split_dv_dqdk): Two separate pallas_calls instead of one fused kernel
        dv = chunk_gla_bwd_dv(q, k, v, g_cumsum, h, do, dh, scale, chunk_size)
        dq, dk = chunk_gla_bwd_dqdk(q, k, v, g_cumsum, h, do, dh, scale, chunk_size)
        return dq, dk, dv
