"""GMM block-wise FP8 quantization kernel for TPU -- L1_bwd_tk1024 variant.
...
This variant increases backward TK from 512 to 1024, doubling the K-tile size
in the backward gmm pass. For bwd_gmm with transpose_rhs=True, the K dimension
of the transposed rhs is the original N dimension. Larger TK reduces loop
iterations in the backward gmm computation.

Tiling changes vs parent:
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 1024, 128) -- TK 512->1024
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,1024,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 1024, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_bwd_tk1024 variant: backward TK 512->1024 to halve K-loop in bwd_gmm.
    fwd (256,512,256), bwd_gmm (256,1024,128), tgmm (2048,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (256, 1024, 128) -- TK 512->1024 to halve K-loop
