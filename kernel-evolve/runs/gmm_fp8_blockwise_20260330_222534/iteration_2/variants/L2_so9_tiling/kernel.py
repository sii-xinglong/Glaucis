"""GMM block-wise FP8 quantization kernel for TPU -- skip_lhs_t_fwd variant.

Builds on SO8 (zero backward quantization, 1.974x) by eliminating the
forward lhs_t quantization call entirely. The baseline quantizes lhs_t
to fp8 in forward, but SO8 showed that tgmm in backward uses bf16 inputs
anyway (SO5). Therefore the forward lhs_t quantization is wasted work.
...
def gmm_fp8_blockwise(
    ...
    skip_lhs_t_fwd variant: eliminates forward lhs_t quantization,
    stores bf16 lhs in residuals, computes lhs_t in backward for tgmm.
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # Use _clamp_tiling instead of min(t, tile_size) to allow tiling > 128 (F004)
    ...
    # SKIP lhs_t quantization entirely -- this is the key optimization.
    # The baseline quantizes lhs_bf16.swapaxes(0,1) to fp8 here, but
    # SO8 showed tgmm uses bf16 anyway. This saves VPU cycles and
    # reduces register pressure (160K spills in baseline).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Tiling (2048, 256, 128) unconstrained for tgmm
