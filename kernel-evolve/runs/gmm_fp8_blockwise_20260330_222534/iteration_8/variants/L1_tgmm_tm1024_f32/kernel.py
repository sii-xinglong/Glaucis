"""GMM block-wise FP8 quantization kernel for TPU -- L1_tgmm_tm1024_f32 variant.

L1_tgmm_tm1024_f32 -- testing tgmm TM=1024 on L1 code path with f32 scales.
Isolates the tgmm TM=1024 effect without bf16 scale interference (FP24 showed
bf16+TM1024 are incompatible).

Builds on L1_fwd_tn256 (which proved phase-specialized tiling with forward
TN=256 is effective). This variant reduces tgmm TM from 2048 to 1024 to test
whether a smaller M-tile in the tgmm pass improves performance.

Tiling changes vs parent (L1_fwd_tn256):
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 128) -- unchanged
  - tgmm:         (1024, 512, 128) -- TM 2048->1024
...
Key changes vs baseline:
  ...
  - scale_dtype=jnp.float32 throughout (no bf16 scale conversion)
...
def gmm_fp8_blockwise(
    ...
    L1_tgmm_tm1024_f32 variant: tgmm TM 2048->1024 on L1 code path with f32 scales.
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # tgmm tiling: (1024, 512, 128) -- TM=1024 (reduced from 2048) to test
    # whether a smaller M-tile improves tgmm performance on the L1 code path.
