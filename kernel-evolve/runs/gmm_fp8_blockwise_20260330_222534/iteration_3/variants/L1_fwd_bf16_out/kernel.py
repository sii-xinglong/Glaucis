"""GMM block-wise FP8 quantization kernel for TPU -- L1_fwd_bf16_out variant.

Builds on L1_fwd_tk512 (2.335x) by changing forward gmm out_dtype from
jnp.float32 to jnp.bfloat16. This halves accumulator register pressure
(bf16 uses half the VMEM per element vs f32) and may allow more MXU ops
per VLIW bundle. The f32->bf16 conversion for backward input is also
eliminated since the forward output is already bf16.

Key change vs L1_fwd_tk512:
  - Forward gmm: out_dtype=jnp.bfloat16 (was jnp.float32)

Everything else identical to L1_fwd_tk512:
  ...
  - Phase-specialized tiling: (256,512,128, 1024,256,128, 2048,512,128)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 128, 1024, 256, 128, 2048, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L1_fwd_bf16_out variant: forward gmm uses out_dtype=bfloat16 to halve
    accumulator register pressure. Phase-specialized tiling with
    fwd (256,512,128), bwd_gmm (1024,256,128), tgmm (2048,512,128).
...
def _gmm_fwd(lhs, rhs, group_sizes, qt_rule, tiling):
    ...
    # out_dtype=bfloat16 halves accumulator VMEM usage vs float32,
    # reducing register pressure and potentially enabling more MXU
    # ops per VLIW bundle. The downstream backward pass receives
    # bf16 directly, eliminating the f32->bf16 conversion.
