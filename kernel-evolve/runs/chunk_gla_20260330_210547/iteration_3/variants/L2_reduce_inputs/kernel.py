...
Mutation: L2_reduce_inputs
  Lineage: L2_single_fused_k_tile -> L2_reduce_inputs
  Motivated by SO11 profile signal: eliminating a separate pallas_call
  (the A computation) from the backward path produced a 25% speedup by
  reducing kernel launch overhead. L2's V-tiled kernel still calls
  chunk_gla_fwd_intra_gk in chunk_gla_bwd_fused. This variant removes
  that call and recomputes A inline within the backward kernel from
  q_pos and k_neg, which are already available.

  Changes:
    1. _chunk_gla_bwd_vtiled_kernel: removed a_ref parameter; recompute
       b_a = q_pos @ k_neg.T * scale ONCE before the V-loop.
    2. chunk_gla_bwd_fused: removed chunk_gla_fwd_intra_gk call, removed
       _A from data prep, reduced in_specs from 8 to 7 inputs.
    3. Forward path and all other code unchanged.
"""
...
# ============================================================
# Backward: Fused dq, dk, dv, dg with V-tiling (Pallas kernel)
#
# Mutation: reduce_inputs
#   Removed a_ref input; A is now recomputed inside the kernel
#   from q_pos and k_neg (which are already computed for other
#   purposes). This eliminates the separate chunk_gla_fwd_intra_gk
#   pallas_call from the backward path, reducing:
#     - One entire kernel launch (host->device dispatch overhead)
#     - HBM read of q,k,g for A computation (redundant with bwd kernel)
#     - HBM write of A tensor + HBM read of A tensor into bwd kernel
#     - Total saved: ~3 HBM round-trips for [H, B*NT, BT, BT] tensor
#
#   The recomputation cost is one [BT,K]@[K,BT] MXU dot (64x128 @ 128x64),
#   which is negligible compared to the V-dimension dots and overlaps
#   with VPU gate computations.
#
# Register pressure reduction via V-dimension tiling:
#   (same V-tiling strategy as parent L2_single_fused_k_tile)
# ============================================================
...
def _chunk_gla_bwd_vtiled_kernel(
    ...
    *, BT, scale,
    ...
    # -------------------------------------------------------
    # Recompute A from q_pos and k_neg (ONCE, before V-loop).
    # This replaces loading a_ref and eliminates the separate
    # chunk_gla_fwd_intra_gk pallas_call from backward path.
    # A = q_pos @ k_neg.T * scale: [BT,K] @ [K,BT] -> [BT,BT]
    # -------------------------------------------------------
    b_a = jnp.dot(
        q_pos, k_neg.T,
        precision=jax.lax.Precision.HIGHEST,
    ...
    # Also accumulate dgk_inter partial sum across V chunks:
    # dgk_inter = exp(gn) * sum_v(h * dh, axis=1) + sum(dk_inter * k, axis=0)
    # The first term is sum over V dimension of h*dh, needs accumulation.
    # The second term uses dk_inter which is itself accumulated, computed after loop.
    dgk_h_dh_acc = jnp.zeros((K,), dtype=jnp.float32)  # sum_v of h*dh row-sums
    ...
    # dgk_inter partial: sum(h_0 * dh_0, axis=1)  ->  [K]
    dgk_h_dh_acc = dgk_h_dh_acc + jnp.sum(b_h_0 * b_dh_0, axis=1)
    ...
    # dgk_inter partial: sum(h_1 * dh_1, axis=1)  ->  [K]
    dgk_h_dh_acc = dgk_h_dh_acc + jnp.sum(b_h_1 * b_dh_1, axis=1)
    ...
    # -------------------------------------------------------
    # dgk_inter uses the accumulated h*dh sum and dk_inter*k sum
    dgk_inter = (jnp.exp(b_gn) * dgk_h_dh_acc
...
def chunk_gla_bwd_fused(q, k, v, g_cumsum, h, do, dh, scale, chunk_size):
    """Launch V-tiled fused backward Pallas kernel (reduce_inputs variant).

    Unlike the parent kernel, this does NOT call chunk_gla_fwd_intra_gk
    to precompute A. Instead, A is recomputed inside the backward kernel
    from q_pos and k_neg, eliminating one pallas_call and one full HBM
    round-trip for the A tensor.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
    ...
    # NOTE: No chunk_gla_fwd_intra_gk call here -- A is recomputed in-kernel
