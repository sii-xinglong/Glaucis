...
# Variant: L1_bf16_h_scratch — bf16 h residual storage for reduced HBM bandwidth
"""fused_chunk_simple_gla Pallas TPU kernel — L1_bf16_h_scratch variant.

Based on bwd_kv_tiling variant. Change: store the h residual (from chunk_fwd_h,
saved for backward) as bf16 instead of f32 by setting states_in_fp32=False.

This halves the HBM bandwidth for reading/writing the h tensor [B,NS,H,K,V]
which is saved as a forward residual and loaded by backward Pass 2 (dq+dk).
The backward kernel already casts h to f32 on load (.astype(jnp.float32)),
so no backward kernel changes are needed.

The h VMEM scratch buffer in the forward fused kernel remains f32 to preserve
accumulation precision (h[t] = g*h[t-1] + k.T@v is an accumulating recurrence).
Only the OUTPUT of chunk_fwd_h (the point-in-time snapshot consumed by backward)
is stored as bf16 — this is a single truncation at the output boundary, proven
safe by prior experiments (SO17).

Forward kernel: 4-step grid unrolling (UNCHANGED from baseline).
Backward: two-phase kernel bodies (UNCHANGED from bwd_kv_tiling).
"""
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _fwd(q, k, v):
        ...
        h, _ = chunk_fwd_h(
            ...
            h0=None, output_final_state=False,
            chunk_size=chunk_size, states_in_fp32=False,
        )
        return o, (q, k, v, h)
