...
#
# MUTATION (recompute_dh): Eliminates the separate chunk_bwd_dh_pallas
# call by fusing dh accumulation into the fused backward kernel.
#
# The original backward used 2 pallas_calls:
#   1. chunk_bwd_dh_pallas  — reverse scan computing dh[t] for all t
#      writes [B, NT, H, K, V] = 256MB to HBM
#   2. chunk_gla_bwd_fused  — reads dh[t] from HBM for each chunk
#
# This mutation merges them into ONE kernel:
#   - Iterates in REVERSE time order (inputs flipped along time axis)
#   - VMEM scratch holds the running dh state [BK, BV]
#   - At each reverse-time step (chunk index t in flipped space):
#       1. Use current dh from scratch for dq/dk/dv computation
#       2. Update dh: dh = dh * state_decay + q_hat.T @ do
#       3. Write dq[t], dk[t], dv[t]
#
# This eliminates:
#   - 1 kernel launch (chunk_bwd_dh_pallas)
#   - 256MB HBM write + read for dh tensor
#   - All HBM allocation for dh intermediate
#
# Grid: (B, H, K/BK, V/BV, NT) with time as "arbitrary"
# Inputs q, k, v, g, h, do are all FLIPPED along time axis before
# the pallas_call (chunk axis=2 for all 5D arrays).
# Outputs dq, dk, dv are FLIPPED back after the pallas_call.
#
# h input also needs to be flipped because dh[t] pairs with h[t]
# (the forward state BEFORE chunk t), and we now process in reverse.
# ============================================================


def _chunk_gla_bwd_fused_recompute_dh_kernel(
    q_ref, k_ref, v_ref, g_ref, h_ref, do_ref, g_gamma,
    dq_ref, dk_ref, dv_ref, scratch_ref,
    *, BT, NT, scale,
):
    """Fused backward kernel: accumulates dh in VMEM and computes dq/dk/dv.
    ...
    At each step:
      ...
      3. Update dh: dh = dh * state_decay + q_hat.T @ do
    """
    BK = q_ref.shape[3]
    BV = do_ref.shape[3]
    i_b, i_h, i_k, i_v, i_t = (
    ...
    # i.e. the END of the original sequence)
    @pl.when(i_t == 0)
    def init():
        scratch_ref[:, :] = jnp.zeros((BK, BV), dtype=jnp.float32)
    ...
    b_h = h_ref[0, 0].astype(jnp.float32)    # [BK, BV]
...
def chunk_gla_bwd_fused_recompute_dh(q, k, v, g_cumsum, h, do, g_gamma, scale, chunk_size):
    """Fused backward: single Pallas kernel combining dh + dq/dk/dv.

    MUTATION (recompute_dh): Replaces chunk_bwd_dh_pallas + chunk_gla_bwd_fused
    with a single pallas_call that accumulates dh in VMEM scratch.

    Eliminates the 256MB dh HBM round-trip and one kernel launch.

    Reverse scan is implemented by flipping all inputs along the chunk
    time axis before the call, and flipping dq/dk/dv after.
    """
    BK, BV, BT = 128, 128, chunk_size
    B, T, H, K = q.shape
    V = v.shape[-1]
    ...
    # Flatten back to (B, H, NT*BT, dim) for BlockSpec compatibility
    ...
    h_flat = h_flipped.reshape(B, H, NT, K, V)   # keep 5D for h_map
    ...
    dq_flat, dk_flat, dv_flat = pl.pallas_call(
        functools.partial(
            _chunk_gla_bwd_fused_recompute_dh_kernel,
        ...
        grid_spec=pltpu.PrefetchScalarGridSpec(
            ...
            in_specs=[
                ...
                pl.BlockSpec((1, 1, BT, BK), k_map),       # k
                pl.BlockSpec((1, 1, BT, BV), v_map),       # v
                pl.BlockSpec((1, 1, BT, BK), g_map),       # g
                pl.BlockSpec((1, 1, BK, BV), h_map),       # h
                pl.BlockSpec((1, 1, BT, BV), do_map),      # do
                pl.BlockSpec(memory_space=pltpu.SMEM),     # g_gamma
            ],
            out_specs=[
                pl.BlockSpec((1, 1, BT, BK), out_k_map),  # dq
                pl.BlockSpec((1, 1, BT, BK), out_k_map),  # dk
                pl.BlockSpec((1, 1, BT, BV), out_v_map),  # dv
            ],
            scratch_shapes=[pltpu.VMEM((BK, BV), jnp.float32)],
        ),
        out_shape=[
    ...
    )(q_flat, k_flat, v_flat, g_flat, h_flat, do_flat, g_gamma)

    # Flip outputs back: they were computed in reverse chunk order
...
# custom_vjp wrapper
#
# MUTATION (recompute_dh + fuse_fwd_combined):
#   _fwd: uses chunk_fwd_combined (single pallas_call for forward)
#   _bwd: uses chunk_gla_bwd_fused_recompute_dh (single pallas_call
...
def chunk_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    def _bwd(residuals, do):
        ...
        # MUTATION (recompute_dh): Single fused pallas_call replaces
        # chunk_bwd_dh_pallas + chunk_gla_bwd_fused (2 calls -> 1 call)
        dq, dk, dv = chunk_gla_bwd_fused_recompute_dh(
