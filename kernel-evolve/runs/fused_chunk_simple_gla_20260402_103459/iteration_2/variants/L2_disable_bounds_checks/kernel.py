...
# Variant: L2_disable_bounds_checks — disable_bounds_checks=True on ALL pallas_calls
"""fused_chunk_simple_gla Pallas TPU kernel — L2 disable_bounds_checks on all pallas_calls.

Fused chunk forward (h + o in single kernel) + monolithic fused backward (single
pallas_call computes dq, dk, dv, and propagates dh in one pass).
...
Backward is a single fused pass that handles dq, dk, dv, and dh propagation
all at once in reverse time order. Eliminates:
  - The dh_states intermediate tensor [B,H,NT,K,V] from HBM
  - 1 pallas_call (2 backward kernels -> 1)

L2 optimization: disable_bounds_checks=True added to ALL three pallas_call
CompilerParams (forward, chunk_fwd_h, backward). This is a compiler directive
that eliminates bounds-checking conditional branches from compiled VLIW code,
freeing registers used for bounds-check temporaries and allowing the compiler
to schedule more aggressively without safety barriers.

Optimization targets within the EVOLVE-BLOCK:
  - Forward kernel: 4-step time unrolling to reduce grid overhead (unchanged)
  - Backward kernel: monolithic single-pass fused dq+dk+dv+dh kernel
  - All pallas_calls: disable_bounds_checks=True compiler directive
"""
...
# =============================================================================
# Backward: Single fused kernel — dq + dk + dv + dh propagation in one pass
# =============================================================================
# This monolithic backward kernel replaces the 2-pass split (Pass 1: dv+dh,
# Pass 2: dq+dk) with a single fused kernel that does everything at once.
#
# Key benefit: the dh_states intermediate tensor [B,H,NT,K,V] never
# materialises in HBM — dh is kept in VMEM scratch throughout.
#
# This also reduces from 2 pallas_calls to 1 in the backward pass.
#
# Adapted from the reference _fused_chunk_bwd_kernel, which is the correct
# monolithic implementation for this recurrence.
...
def _fused_chunk_bwd_kernel(
    ...
    *,
    ...
    Merges dh propagation and dq/dk/dv computation so that the full dh
    tensor [B, NT, H, K, V] never materialises in HBM.
...
def _fused_chunk_bwd_launcher(
    ...
    """Single-pass monolithic backward launcher.

    Replaces the 2-pass split (Pass 1: dv+dh, Pass 2: dq+dk) with a single
    fused kernel. The dh_states intermediate tensor [B,H,NT,K,V] is eliminated
    — dh is kept in VMEM scratch across the NT sequential backward steps.

    This reduces backward from 2 pallas_calls to 1.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    BT = chunk_size
...
def fused_chunk_simple_gla(q, k, v, g_gamma, scale, chunk_size):
    ...
    Backward uses a single monolithic fused kernel that computes dq, dk, dv,
    and propagates dh all at once. Eliminates the dh_states intermediate
    tensor [B,H,NT,K,V] from HBM and reduces backward from 2 pallas_calls
    to 1.
    """
    @jax.custom_vjp
    def _compute(q, k, v):
