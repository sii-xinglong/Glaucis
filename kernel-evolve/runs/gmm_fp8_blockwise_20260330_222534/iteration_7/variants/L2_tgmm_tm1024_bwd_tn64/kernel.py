"""GMM block-wise FP8 quantization kernel for TPU -- L2_tgmm_tm1024_bwd_tn64 variant.

Builds on L2_tgmm_tm1024 which decreased tgmm TM from 2048 to 1024 with
phase-specialized tiling (256,512,256, 256,512,128, 1024,512,128).

This variant decreases bwd_gmm TN from 128 to 64, testing if smaller TN
reduces register pressure for the backward gmm pass. bwd TN=256 caused
catastrophic spills (FP16b), so the direction might be SMALLER TN rather
than larger. TN=64 halves the intermediate buffer compared to TN=128.

Tiling changes vs parent (L2_tgmm_tm1024):
  - Forward gmm:  (256, 512, 256) -- unchanged
  - Backward gmm: (256, 512, 64)  -- TN 128->64
...
Key changes vs baseline:
  ...
  - Phase-specialized tiling: (256,512,256, 256,512,64, 1024,512,128)
  - _clamp_tiling lower bound reduced to 64 to allow bwd TN=64
...
def _clamp_tiling(tiling, tile_size):
    """Clamp each tiling dimension to be >= 64 (the minimum) but allow > tile_size.
    ...
    Lower bound reduced from 128 to 64 to allow bwd TN=64.
    """
    return tuple(max(t, 64) for t in tiling)
...
def gmm_fp8_blockwise(
    ...
    tiling: tuple[int, ...] = (256, 512, 256, 256, 512, 64, 1024, 512, 128),
) -> jnp.ndarray:
    """GMM with fp8_blockwise quantization and tokamax backend.

    L2_tgmm_tm1024_bwd_tn64 variant: bwd_gmm TN 128->64 to test reduced
    register pressure. bwd TN=256 caused catastrophic spills (FP16b), so
    the direction might be SMALLER TN. TN=64 halves the intermediate buffer.
    fwd (256,512,256), bwd_gmm (256,512,64), tgmm (1024,512,128).
...
def _gmm_bwd(lhs_dtype, rhs_dtype, qt_rule, tiling, residual, grad):
    ...
    # Backward gmm tiling: (256, 512, 64) -- TM=256 for per-group alignment,
    # TK=512 halves K-loop iterations (K=2048: 4 iters instead of 8),
    # TN=64 halves intermediate buffer vs TN=128 to reduce register pressure
    ...
    # tgmm tiling: (1024, 512, 128) -- TM 2048->1024 to test reduced overhead.
    # Smaller TM may reduce register pressure in the tgmm M-loop.
